...
public class RiskManageAction extends BasePageAction {
	...
	public String queryRiskList() {
		...
		String resource = paramMap.get("resource") == null ? "" : paramMap
				.get("resource");
		String timeStart = paramMap.get("timeStart") == null ? "" : paramMap
				.get("timeStart");
		String timeEnd = paramMap.get("timeEnd") == null ? "" : paramMap
				.get("timeEnd");
		String riskType = paramMap.get("riskType") == null ? "" : paramMap
				.get("riskType");
		try {
			fundManagementService.queryRiskByCondition(resource, timeStart,
					timeEnd, riskType, pageBean);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
		} 
	...
	public String exportRiskList() {
		...
		try {
			String resource = Convert.strToStr(request.getString("resource"), "");
			resource = URLDecoder.decode(resource, "UTF-8");
			String timeStart = Convert.strToStr(request.getString(
					"timeStart"), null);
			String timeEnd = Convert.strToStr(
					request.getString("timeEnd"), null);
			String riskType = Convert.strToStr(request.getString(
					"riskType"), "");
			...
			HSSFWorkbook wb = ExcelUtils.exportExcel("风险保障金记录", pageBean
					...
					"操作时间" }, new String[] { "amount", "realName", "riskType",
					"resource", "riskDate" });
			this.export(wb, new Date().getTime() + ".xls");
			Admin admin = (Admin) session().getAttribute(
					IConstants.SESSION_ADMIN);
			operationLogService.addOperationLog("v_t_risk_list_h", admin
	 ...
	 * @Throws:
	 */
	public String queryRiskDetail() throws Exception {
		String id = request.getString("id") == null ? "" : request.getString("id");
		long idLong = Convert.strToLong(id, -1L);
		Map<String, String> riskDetailMap = fundManagementService
				.queryRiskDetailById(idLong);
		if(riskDetailMap != null && StringUtils.isBlank(riskDetailMap.get("trader"))){
			riskDetailMap.put("trader", IConstants.PRO_GLOBLE_NAME);
		}
	 ...
	 * @Throws:
	 */
	public String addRisk() throws Exception {
		...
		long returnid = -1;
		returnid = fundManagementService.addRisk(amountDouble, admin.getId(),
				remark);
		if (returnid <= 0) {
			obj.put("msg", IConstants.ACTION_FAILURE);
			JSONUtils.printObject(obj);
			operationLogService.addOperationLog("t_risk_detail", admin
					.getUserName(), IConstants.INSERT, admin.getLastIP(),
					amountDouble, "手动添加风险保障金失败", 2);
			return null;
		}
		obj.put("msg", "1");
		JSONUtils.printObject(obj);
	 ...
	 * @Throws:
	 */
	public String deductedRisk() throws Exception {
		Admin admin = (Admin) session().getAttribute("admin");
		JSONObject obj = new JSONObject();
		String amount = paramMap.get("amount");
		double amountDouble = Convert.strToDouble(amount, -1);
		...
		long returnid = -1;
		returnid = fundManagementService.deductedRisk(amountDouble, admin
				.getId(), remark);
		if (returnid <= 0) {
			obj.put("msg", IConstants.ACTION_FAILURE);
			JSONUtils.printObject(obj);
