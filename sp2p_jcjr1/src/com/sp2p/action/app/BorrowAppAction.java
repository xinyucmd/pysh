...
public class BorrowAppAction extends BaseAppAction {
	...
	public String addBorrowInit() throws IOException {
       ...
       if(IConstants.BORROW_TYPE_INSTITUTION_GUARANTEE.equals(borrowWay + "")){
    	   Map<String, String> map = dataApproveService.querySauthId(userId,
			...
			jsonMap.put("deadlineDayList", shoveBorrowTypeService
					.queryDeadlineDaythListByNid(nid));
	...
	public String addBorrow() throws IOException {
		...
		try {
			...
			int amount = Convert.strToInt(appInfoMap.get("amount"), 0);
	        ...
	        if (number != subscribe_status) {
		       jsonMap.put("error", "13");
		       jsonMap.put("msg", "该种类型借款已启用认购模式，app端暂不支持该模式，请用电脑登录发布借款");
		       JSONUtils.printObject(jsonMap);
		        return null;
	         }
	        //总额范围
			...
			boolean res = userService.checkSign(userId);
			if(!res){
			...
			//whb 添加是否专享标
			int isExclus = Convert.strToInt(appInfoMap.get("isExclus"), 0);
			...
			returnMap = borrowService.addBorrow(title, imgPath, borrowWay,
