...
public class InvestAppAction extends BaseAppAction {
	...
	public String queryInvestList() throws IOException {
		...
		try {
			...
			String paging = appInfoMap.get("paging");
			...
			if(getUserAgent()==1){
				...
				financeService.queryBorrowByCondition(borrow_t,borrowStatus, borrowWay,
				...
				if(paging != null && !"".equals(paging)){
					...
					List<Map<String, Object>> lists = financeService.queryBorrowAll(borrow_t,borrowStatus, borrowWay, title,
							paymentMode, purpose, deadline, reward, arStart, arEnd,	IConstants.SORT_TYPE_DESC);
					jsonMap.put("list", lists);
				}
			}
...
//				}
			...
			Map<String, String> result = financeService.addBorrowInvest(
					borrowId, userId, "", amountDouble, getBasePath(), userMap
							.get("username"), status, num,0,0,0,isUseHb_6_24);
