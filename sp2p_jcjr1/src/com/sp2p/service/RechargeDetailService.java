...
public class RechargeDetailService extends BaseService {
	 ...
	 * @throws SQLException
	 */
	public long deleteRechargeDetail(String ids) throws Exception {
		Connection conn = MySQL.getConnection();
		long result = -1;
		try {
	 ...
	 * @throws ParseException
	 */
	public int usergoPay(Map<String, Object> paramMap) throws Exception {
		int returnId = -1;
	...
	private int userPayIn(Long orderId, Long userId, BigDecimal total_fee,
		...
		try {
			...
			long rechargeId = Convert
					.strToLong(detailMap.get("rechargeId"), -1);
			...
			if (detailMap != null) {
				if ("1".equals(detailMap.get("result"))) {
					...
					long resultCount = rechargeDetailDao.updateRechargeDetail(
							conn, orderId, paramMap);
					if (resultCount > 0) {
						if (uId == -1) {
							conn.rollback();
							return -3;// 用户编号错误
						...
						fundMap
								.put("usableSum", userAmountMap
										.get("usableSum"));
						fundMap
								.put("freezeSum", userAmountMap
										.get("freezeSum"));
	...
	public int userIPay(Map<String, Object> paramMap) throws Exception {
		...
		try {
			...
			String paybank = (String) paramMap.get("paybank");
			String[] paras = Encrypt.decryptSES(paramMap.get("attach") + "",
					IPayConfig.ipay_see_key).split("_");
			if (paras == null || paras.length != 2) {
				return -2;
			}
			Long orderId = Convert.strToLong(paras[0], -1);// 获得订单编号
			if (orderId < 0) {
				return -2;// 订单编号错误
			}
