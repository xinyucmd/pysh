...
public class BeVipService extends BaseService {
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryIsIdno(String IDNOM,long userId) throws Exception {
		Connection conn = MySQL.getConnection();

		Map<String, String> map = new HashMap<String, String>();
		try {
			//线下借款人不校验身份证重复性
			Map<String, String> reMap = beVipDao.queryUserType(conn, userId);
			if(reMap!=null && reMap.size()>0){
				return map;
			}
			map = beVipDao.queryIsIdno(conn, IDNOM);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public Long updataUserVipStatus(Long userId, int vipStatus,
		...
		vipFee = Utility.filteSqlInfusion(vipFee);
		...
		try {
			...
			if (resultId <= 0) {
				...
				// 发站内信
				resultId = sendmsgService.sendCheckMail(conn,userId, " 申请vip审核通知",
