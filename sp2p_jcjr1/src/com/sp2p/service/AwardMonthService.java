...
public class AwardMonthService extends BaseService {
	 ...
	 * @throws SQLException
	 */
	public void quereyGroupCloseMoneyInfo(
			PageBean<Map<String, Object>> pageBean, String startTime,
			String endTime, int tyle, String username, String realName)
			throws Exception {
		...
		try {
			dataPage(conn, pageBean, "v_t_award_month", "*", "", condition
					.toString());
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	 ...
	 * @throws SQLException
	 */
	public Map<String, String> quereyGroupCloseMoneySum(String startTime,
			String endTime, int tyle, String username, String realName)
			throws Exception {
		startTime = Utility.filteSqlInfusion(startTime);
		endTime = Utility.filteSqlInfusion(endTime);
		...
		try {
			ds = MySQL.executeQuery(conn,
					" select sum(moneys ) as moneySum  from  v_t_award_month where 1=1 "
							+ condition);
