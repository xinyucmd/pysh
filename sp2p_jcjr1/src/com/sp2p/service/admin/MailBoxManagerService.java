...
public class MailBoxManagerService extends BaseService {
	 ...
	 */
	public void queryMailByCondition(PageBean<Map<String, Object>> pageBean, Integer mailType,
			String sender, String beginTime, String endTime) throws Exception {
		sender = Utility.filteSqlInfusion(sender);
	 ...
	 * @return int
	 */
	public int deleteMailBox(String commonIds, String delimiter)
			throws Exception {
		Connection conn = MySQL.getConnection();
		int result = -1;
		try {
			result = mailBoxManagerDao
					.deleteMailBox(conn, commonIds, delimiter);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public Long updateMailBoxById(Long id, String title, String content)
		...
		Long result = -1L;
		try {
			result = mailBoxManagerDao.updateMailBoxById(conn, id, title,
					content);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
	...
	public Map<String, String> getMailById(Long id) throws Exception {
		Connection conn = MySQL.getConnection();
