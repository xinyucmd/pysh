...
public class RechargebankService extends BaseService {
	...
	public long addRechargeBankInit(String bankname, String Account,
			String accountbank, String bankimage, String accountname, long adminId, String adminIp)
			throws Exception {
		long result = -1;
		Connection conn = MySQL.getConnection();
		try {
			result = rechargebankDao.addRechargeBankInit(conn, bankname,
					Account, accountbank, bankimage, accountname, adminId, adminIp);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
     ...
     * @throws SQLException
     * @autthor linww
     * 2014-6-30 下午01:49:37
     */
	public void delRechargeBankInit(long id, Long adminId, String adminIp) throws SQLException {
		Connection conn = MySQL.getConnection();
		try{
		rechargebankDao.delRechargeBankById(conn, id, adminId, adminIp);
		conn.commit();
		} catch (Exception e) {
		log.error(e);
		e.printStackTrace();
