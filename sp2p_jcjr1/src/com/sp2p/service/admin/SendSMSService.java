...
public class SendSMSService extends BaseService {
	...
	public Long insertSmsRecord(String phone,String content,String methods,String ipAdder) throws Exception {
		Connection conn = MySQL.getConnection();
		Long result = 0L;
		try {
			result = sendSMSDao.insertSmsRecord(conn, phone,content,methods,ipAdder);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public List<Map<String,Object>> getNowDaySmsRecordList(String phone) throws Exception {
		Connection conn = MySQL.getConnection();
		List<Map<String,Object>> listMap = null;
		try {
			if(!phone.equals("18210760262")||!phone.equals("13718853545")){
				 listMap = sendSMSDao.getNowDaySmsRecordList(conn, phone);
			}
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
