...
public class SEOConfigService extends BaseService {
	...
	public long updateOptions(long id,int app_state) throws Exception {
		Connection conn = MySQL.getConnection();
		long result = 0L;
		try {
			result = SEOConfigDao.updateOptions(conn,id,app_state);
			if (result <= 0) {
				conn.rollback();
			} else {
				conn.commit();
	...
	public Map<String, String> queryOptions(long id) throws SQLException,DataException {
        ...
        Map<String, String> map = new HashMap<String, String>();
		try {
			map = SEOConfigDao.queryOptions(conn, id);
		} catch (Exception e) {
			e.printStackTrace();
			conn.rollback();
		} finally {
