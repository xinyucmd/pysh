...
public class LinksDao {
	...
	public Long addIndexRollImg(Connection conn,String companyImg,
			Long serialCount,String dateTime,Long ordershort,int cardStatus,String companyURL,int ImgType) throws SQLException{
		Dao.Tables.t_links tlinks = new Dao().new Tables().new t_links();
		if(companyImg != null)
			tlinks.companyImg.setValue(companyImg);
		if(serialCount != null && serialCount > 0)
		...
		if(companyURL != null)
			tlinks.companyURL.setValue(companyURL);
		if(ImgType > -1)
			tlinks.ImgType.setValue(ImgType);
	...
	public Long deleteLinks(Connection conn,String ids) throws SQLException{
		...
		return t_links.delete(conn, "type = 1 and serialCount in("+idSQL+")");
	}
	
	public Long deleteIndexRollImg(Connection conn,String ids) throws SQLException{
		ids=com.shove.web.Utility.filteSqlInfusion(ids);
		String idStr = StringEscapeUtils.escapeSql("'"+ids+"'");
		String idSQL = "-2";
		idStr = idStr.replaceAll("'", "");
	...
	public Map<String, String> queyLinksInfoById(Connection conn, long id) throws SQLException, DataException {
		Dao.Tables.t_links t_links = new Dao().new Tables().new t_links();
		DataSet dataSet = t_links.open(conn, "", "type = 1 and serialCount=" + id, "", -1, -1);
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queyLinksInfoByIds(Connection conn, long id) throws SQLException, DataException {
		Dao.Tables.t_links t_links = new Dao().new Tables().new t_links();
		DataSet dataSet = t_links.open(conn, "", "type != 1 and serialCount=" + id, "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	...
	public Long updateLinks(Connection conn,String companyName,String companyImg,String companyURL,
		...
		return t_links.update(conn, "type = 1 and serialCount="+serialCount+ " and type = 1");
	}
	...
	public Long updateIndexRollImg(Connection conn,String companyImg,
			Long serialCount,Long ordershort,String dateTime,String companyURL,int ImgType) throws SQLException{
		Dao.Tables.t_links t_links = new Dao().new Tables().new t_links();
		if(companyImg!=null){
			t_links.companyImg.setValue(companyImg);
		}
		...
		if(ImgType > -1){
			t_links.ImgType.setValue(ImgType);
		}
		return t_links.update(conn, "type != 1 and serialCount="+serialCount + " and type = 3");
	}
