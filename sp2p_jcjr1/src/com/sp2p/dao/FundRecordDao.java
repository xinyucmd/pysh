...
public class FundRecordDao {
	...
	*/
	...
	public Map<String, String> queryUserIdByWId(Connection conn, long wid) throws DataException, SQLException {
		StringBuffer  condition = new StringBuffer();
		condition.append("select userId from t_withdraw a where id = "+wid);
		DataSet dataSet = MySQL.executeQuery(conn, condition.toString());
		return BeanMapUtils.dataSetToMap(dataSet);
	}
}
