...
public class BorrowManageDao {
	...
	*/
	 ...
	 * @throws Exception
	 */
	public Map<String,String> dataToCsai(Connection conn,long id) throws DataException, SQLException{
		Dao.Views.v_t_borrow_csai t_borrow = new Dao().new Views().new v_t_borrow_csai();
		DataSet dataSet = t_borrow.open(conn, " * ", " p2p_product_id= "+id, "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	 ...
	 * @throws Exception
	 */
	public List<Map<String,Object>> updateCsai(Connection conn) throws DataException, SQLException{
		Dao.Views.v_t_borrow_csai_update v_t_borrow_csai_update = new Dao().new Views().new v_t_borrow_csai_update();
		StringBuffer sql = new StringBuffer();
		sql.append(" push_id, invest_amount,inverst_mans,underlying_end,product_state,repay_start_time,repay_end_time,p2p_product_id,`status` ");

		DataSet dataSet = v_t_borrow_csai_update.open(conn, sql.toString() , " `status` != 4", "", -1, -1);
...
//			DataSet dataSet = t_borrow.open(conn, " publisher,borrowTitle,annualRate,paymentMode,borrowAmount,deadline,borrowWay,excitationType,excitationSum,isDayThe ", "  id="+id, "", 0, 1);
		...
		*/
		...
		public long addBorrowPushHis(Connection conn, long borrowId,int status) throws SQLException {
			Dao.Tables.t_borrow_push_his t_borrow_push_his = new Dao().new Tables().new t_borrow_push_his();
			t_borrow_push_his.borrow_id.setValue(borrowId);
			t_borrow_push_his.status.setValue(status);
			t_borrow_push_his.create_time.setValue(new Date());
			return t_borrow_push_his.insert(conn);
		}
		
		public long updateBorrowPushHis(Connection conn,long id,int status) throws SQLException {
			Dao.Tables.t_borrow_push_his t_borrow_push_his = new Dao().new Tables().new t_borrow_push_his();
			if(status>0){
				t_borrow_push_his.status.setValue(status);
			}
			t_borrow_push_his.modify_time.setValue(new Date());
			return t_borrow_push_his.update(conn, " id= "+id);
		}
		...
		*/
		...
		public Map<String, String> queryUserBaseInfo(Connection conn,long applyId) throws SQLException, DataException{
			Dao.Tables.t_borrow_role t_borrow_role = new Dao().new Tables().new t_borrow_role();
			DataSet dataSet = t_borrow_role.open(conn, "*", " applyId = "+applyId, "", 0, 1);
			return BeanMapUtils.dataSetToMap(dataSet);
		}
		 ...
		 */
		public long deleteApplyById(Connection conn, long id )
		throws SQLException, DataException {
			Dao.Tables.t_apply t_apply = new Dao().new Tables().new t_apply();
			return t_apply.delete(conn, "id=" + id);
		}
		public Map<String, String> queryBorrowFistAuditDetailById2(Connection conn,long id) throws SQLException, DataException{
			DataSet dataSet = MySQL.executeQuery(conn,"select `a`.`id` AS `id`,`b`.`id` AS `userId`,`b`.`username` AS `username`,`c`.`realName` AS `realName`,`a`.`borrowTitle` AS `borrowTitle`,`a`.`borrowAmount` AS `borrowAmount`,`a`.`annualRate` AS `annualRate`,`a`.`deadline` AS `deadline`,`a`.`isDayThe` AS `isDayThe`,`d`.`selectName` AS `purpose`,`a`.`paymentMode` AS `paymentMode`,`a`.`borrowWay` AS `borrowWay`,`a`.`minTenderedSum` AS `minTenderedSum`,`a`.`maxTenderedSum` AS `maxTenderedSum`,`a`.`excitationType` AS `excitationType`,`a`.`excitationSum` AS `excitationSum`,`a`.`excitationMode` AS `excitationMode`,`a`.`hasPWD` AS `hasPWD`,`a`.`publishTime` AS `publishTime`,`a`.`publishIP` AS `publishIP`,`a`.`detail` AS `detail`,`a`.`borrowStatus` AS `borrowStatus`,`a`.`auditOpinion` AS `auditOpinion`,`f`.`mailContent` AS `mailContent`,`a`.`smallestFlowUnit` AS `smallestFlowUnit`,`a`.`circulationNumber` AS `circulationNumber`,`a`.`nid_log` AS `nid_log`,`a`.`borrowShow` AS `borrowShow`,`a`.`businessDetail` AS `businessDetail`,`a`.`assets` AS `assets`,`a`.`counterAgent` AS `counterAgent`,`a`.`moneyPurposes` AS `moneyPurposes` from (((((`t_borrow` `a` left join `t_user` `b` on((`a`.`publisher` = `b`.`id`))) left join `t_person` `c` on((`c`.`userId` = `b`.`id`))) left join `t_select` `d` on((`a`.`purpose` = `d`.`selectValue`))) left join `t_selecttype` `e` on((`d`.`typeId` = `e`.`id`))) left join `t_mail` `f` on((`f`.`borrowId` = `a`.`id`))) where ((`a`.`borrowStatus` = 1) and (`d`.`typeId` = 1)) and b.id = "+id);
			return BeanMapUtils.dataSetToMap(dataSet);
		}
}
