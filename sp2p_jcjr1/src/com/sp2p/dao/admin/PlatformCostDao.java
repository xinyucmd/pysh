...
public class PlatformCostDao {
		 ...
		 * @throws SQLException 
		 */
		public void  queryPlatformCostAll(Connection  conn,PageBean<Map<String,Object>> pageBean,int type) throws SQLException, DataException 
		{
			Dao.Tables.t_platform_cost  t_platform_cost = new Dao().new Tables().new t_platform_cost();
			long c=t_platform_cost.getCount(conn, " types = "+type);//查询当前条件下的所有记录
			boolean  result=pageBean.setTotalNum(c);//-------->将总页数(c)放到PageBean<T>中
			if(result)
			{
				// pageBean.getPageSize()-->  在类PageBean<T>的构造方法中已经赋值  
				DataSet ds= t_platform_cost.open(conn, " * ", " types = "+type, " ", pageBean.getStartOfPage(), pageBean.getPageSize());
		 ...
		 * @throws DataException 
		 */
		public Map<String,String> queryPlatformCostByAlias(Connection  conn, String alias) throws SQLException, DataException
		{
			Dao.Tables.t_platform_cost  t_platform_cost = new Dao().new Tables().new t_platform_cost();
			DataSet  ds=t_platform_cost.open(conn, " * ", " alias = '"+alias+"'", "", -1, -1);
