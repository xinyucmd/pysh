...
public class FIManageDao {
	...
	public Map<String,String> queryPlatformCost(Connection conn) throws SQLException, DataException{
		Dao.Tables.t_platform_cost t_cost = 
				new Dao().new Tables().new t_platform_cost();
		DataSet ds = t_cost.open(conn, "*", "id=9", "", -1, -1);
		return BeanMapUtils.dataSetToMap(ds);
	}
	...
	public Long updateWithdraws(Connection conn,String wids,int status,Long adminId) throws SQLException{
		wids=com.shove.web.Utility.filteSqlInfusion(wids);
