...
public class GroupDao {
	 ...
	 * @throws SQLException
	 */
	public long updateGroup(Connection conn,long id,long adminId,String groupName,String groupRemark,int cashStatus,double transMoney) throws SQLException{
		Dao.Tables.t_group t_group = new Dao().new Tables().new t_group();
		if(adminId > 0){
			t_group.adminId.setValue(adminId);
		}
		...
		if(transMoney > 0){
			t_group.transMoney.setValue(transMoney);
		}
	...
	public long deleteGroup(Connection conn,String id) throws SQLException{
		id=com.shove.web.Utility.filteSqlInfusion(id);
	...
	public List<Map<String, Object>> queryAllGroup(Connection conn) throws SQLException, DataException {
		Dao.Tables.t_group t_group = new Dao().new Tables().new t_group();
		DataSet dataSet=t_group.open(conn, "id,groupName,transMoney", " ", "", -1, -1);
