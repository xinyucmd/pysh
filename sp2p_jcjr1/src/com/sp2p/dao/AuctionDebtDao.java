...
public class AuctionDebtDao {
	...
	public long addAuctionDebt(Connection conn, Map<String, String> paramMap) throws SQLException {
		...
		DBReflectUtil.mapToTableValue(t_invest_debt, paramMap);
		return t_invest_debt.insert(conn);
	 ...
	 * @throws SQLException
	 */
	public long addAuctionDebtNew(Connection conn, Map<String, String> paramMap) throws SQLException {
		Dao.Tables.t_invest_debt_new t_invest_debt_new = new Dao().new Tables().new t_invest_debt_new();
		DBReflectUtil.mapToTableValue(t_invest_debt_new, paramMap);
		return t_invest_debt_new.insert(conn);
	...
	public long updateAuctionDebt(Connection conn, long id,Map<String, String> paramMap) throws SQLException {
		Dao.Tables.t_invest_debt t_invest_debt = new Dao().new Tables().new t_invest_debt();
		DBReflectUtil.mapToTableValue(t_invest_debt, paramMap);
		return t_invest_debt.update(conn, "id=" +id);
	...
	public long deleteAuctionDebt(Connection conn, String ids) throws SQLException{
		...
		Dao.Tables.t_invest_debt t_invest_debt = new Dao().new Tables().new t_invest_debt();
		return t_invest_debt.delete(conn, " id in(" + idSQL + ")");
	}
	...
	public Map<String,String> getAuctionDebt(Connection conn, long id) throws SQLException, DataException{
		Dao.Tables.t_invest_debt t_invest_debt = new Dao().new Tables().new t_invest_debt();
		DataSet ds = t_invest_debt.open(conn, "*", "id="+id, "", -1, -1);
	 ...
	 * @throws SQLException 
	 */
	public List<Map<String, Object>> queryAuctionDebtByDebtId(Connection conn,
			long debtId) throws SQLException, DataException {
		Dao.Views.v_t_invest_debt_user v_t_invest_debt_user= new Dao().new Views().new v_t_invest_debt_user();
		DataSet ds = v_t_invest_debt_user.open(conn, " id  ,  debtId  , createTime ,   debtPrice  ,f_formatting_username(username) as username  , auctionerId   ", "debtId="+debtId, "", -1, -1);
	...
	public Map<String, String> queryAuctionMaxPriceAndCount(Connection conn,long debtId) throws SQLException, DataException {
		...
		Dao.Tables.t_invest_debt t_invest_debt = new Dao().new Tables().new t_invest_debt();
		DataSet ds = t_invest_debt.open(conn, "id,userId,debtPrice", conditon.toString(), " debtPrice desc", -1, -1);
		ds.tables.get(0).rows.genRowsMap();
		...
		if(list != null && list.size() > 0){
			...
			map.put("maxAuctionPrice",auctionMap.get("debtPrice")+"" );
			t_invest_debt = new Dao().new Tables().new t_invest_debt();
			ds = t_invest_debt.open(conn, "count(1) as counts", conditon.toString(), "", -1, -1);
	...
	public Map<String, String> getUserAddressById(Connection conn, long userId) throws SQLException, DataException {
		...
		DataSet ds = v_t_user_address.open(conn, "  f_formatting_username( username)   as username, lastDate  ,  createTime  , id  , address  , rating  , creditratingIco  , ratingIco  ,creditrating  , personalHead   ", "id="+userId, "", -1, -1);
		return BeanMapUtils.dataSetToMap(ds);
	}
	...
	public long queryAuctionUserTimes(Connection conn, long debtId, long userId) throws SQLException, DataException {
		...
		Dao.Tables.t_invest_debt t_invest_debt = new Dao().new Tables().new t_invest_debt();
		DataSet ds = t_invest_debt.open(conn, "count(1) as autiontimes", conditon.toString(), "", -1, -1);
	 ...
	 * @throws SQLException 
	 */
	public Map<String, String> queryBorrowerImgpath(Connection conn, long borrowId) throws SQLException, DataException {
		Dao.Tables.t_borrow t_borrow = new Dao().new Tables().new t_borrow();
		DataSet ds = t_borrow.open(conn, "*", " id="+borrowId, "", -1, -1);
		return BeanMapUtils.dataSetToMap(ds);
	}
	 ...
	 * @throws SQLException 
	 */
	public Map<String, String> queryTermByBorrowId(Connection conn, long borrowId, long investId) throws SQLException, DataException {
		Dao.Tables.t_invest_repayment t_invest_repayment = new Dao().new Tables().new t_invest_repayment();
		StringBuilder conditon = new StringBuilder(" repayStatus = 1 ");
		if(borrowId > 0){
			conditon.append(" and borrow_id=");
			conditon.append(borrowId);
		}
		if(investId > 0){
			conditon.append(" and invest_id=");
			conditon.append(investId);
		}
		DataSet ds = t_invest_repayment.open(conn, " MIN(id) AS id, COUNT(1) AS term", conditon.toString(), "", -1, -1);
		return BeanMapUtils.dataSetToMap(ds);
	}


	public Map<String, String> getAuctionDebt(Connection conn, long debtId,
			long userId) throws SQLException, DataException {
		Dao.Tables.t_invest_debt t_invest_debt = new Dao().new Tables().new t_invest_debt();
		DataSet ds = t_invest_debt.open(conn, "*", "debtId="+debtId +" and userId="+userId, "", -1, -1);
		return BeanMapUtils.dataSetToMap(ds);
	}
