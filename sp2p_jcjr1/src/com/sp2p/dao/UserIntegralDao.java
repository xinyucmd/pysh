...
public class UserIntegralDao {
	...
	public Map<String, String> queryUserIntegral2(Connection conn,
		...
		intergraltype=com.shove.web.Utility.filteSqlInfusion(intergraltype);
		command.append("SELECT min(id) as minId,changerecore FROM t_userintegraldetail WHERE 1=1 AND userid="+userid+" AND type=2 AND intergraltype = \""+StringEscapeUtils.escapeSql(intergraltype)+"\"");
