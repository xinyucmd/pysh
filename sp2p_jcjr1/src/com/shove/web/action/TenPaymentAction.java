...
public class TenPaymentAction extends BaseFrontAction {
	...
	public String tenPayment() throws Exception {
		...
		String bankType = request.getString("bank_type");
	...
	public String tenPayReceive() throws Exception {
		...
		if (isTenpaySign(para)) {
			...
			if (!"0".equals(request.getString("trade_state"))) {
				createHelpMessage("支付失败！", "返回首页", "index.do");
			}
...
//			log.info("--tenpay--3--");
			...
			String paybank = TenPayConfig.bankMap.get(request.getString("bank_type"));// 支付银行
...
//			map.put("notify_time", notify_time);
			...
			double money = request.getDouble("total_fee", -1);
			String in_paynumber = request.getString("out_trade_no");
	...
	public String tenPayNotify() throws Exception {
		...
		try {
			...
			String extra_common_param = request.getString("attach");// 获得参数信息
			// 支付类型_订单编号/金钱_用户编号
