...
public class IPaymentAction extends BasePageAction<IPaymentAction>{
	...
	public String ipayPayment() throws Exception {
		...
		String gatewayType = request.getString("gatewayType");
		...
		// 订单号
		long number = Convert.strToLong(result.get("result"), -1);

		if (number != -1) {
			String html = createIpayUrl("在线充值", number, userId,
	...
	private boolean validateSign() {
		...
		sign.append("billno");
		sign.append(request.getString("billno"));
		sign.append("currencytype");
		sign.append(request.getString("Currency_type"));
		sign.append("amount");
		sign.append(request.getString("amount"));
		sign.append("date");
		sign.append(request.getString("date"));
		sign.append("succ");
		sign.append(request.getString("succ"));
		sign.append("ipsbillno");
		sign.append(request.getString("ipsbillno"));
		sign.append("retencodetype");
		sign.append(request.getString("retencodetype"));
		sign.append(IPayConfig.ipay_certificate);
		MD5 md5 = new MD5();
		log.info("sign==>"+sign);
		String signMd5 = md5.toMD5(sign.toString()).toLowerCase();
		log.info("signMd5==>"+signMd5);
		log.info("signature==>"+request.getString("signature"));
		return signMd5.equals(request.getString("signature"));
...
//		ServletContext application = ServletActionContext.getServletContext();
	...
	public String merServerUrl() throws Exception {
		...
		// 环迅支付订单号查询
		// createIpayUrl();
		String respCode = request.getString("succ");// 
		log.info("2--" + respCode);
...
//		map.put("succ", request("succ"));
		String attach = request.getString("attach");
		double money = request.getDouble("amount", -1);
		String in_paynumber = request.getString("billno");
