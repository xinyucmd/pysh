...
public class GroupService extends BaseService {
	...
	public long addGroup(long adminId, List<Long> userIds, String groupName,
			String groupRemark, int cashStatus) throws Exception {
		Connection conn = MySQL.getConnection();
		long result = -1;
		try {
			result = groupDao.addGroup(conn, adminId, groupName, groupRemark,
					cashStatus);
	 ...
	 * @throws Exception
	 */
	public long deleteGroup(String id) throws Exception {
		Connection conn = MySQL.getConnection();
		long result = -1;
		try {
	 ...
	 * @throws Exception
	 */
	public long deleteGroupUsersByIds(String ids) throws Exception {
		Connection conn = MySQL.getConnection();
		long result = -1;
		try {
	 ...
	 */
	public long addGroupUser(List<Long> userIds, long groupId) throws Exception {
		Connection conn = MySQL.getConnection();
		long result = -1;
		try {
			if (userIds != null) {
	 ...
	 * @throws Exception
	 */
	public Map<String, String> getGroup(long id) throws Exception {
		Connection conn = MySQL.getConnection();
	...
	/**
	 * 分页查询用户组信息 -----modify by houli 添加一个groupName参数，用来进行用户组搜索
	 * 
	 * @param pageBean
	 * @throws Exception
	 * @throws DataException
	 */
	public void queryGroupForPage(PageBean<Map<String, Object>> pageBean, String groupName)
			throws Exception {
	...
	public void queryGroupUserForPage(String userName, String realName,
			...
			double endUseableSum, long cashStatus, PageBean<Map<String, Object>> pageBean)
			throws Exception {
	...
	public void queryGroupUsersForPage(String userName, String realName,
			double startAllSum, double endAllSum, double startUseableSum,
			double endUseableSum, long groupId, PageBean<Map<String, Object>> pageBean)
			throws Exception {
		userName = Utility.filteSqlInfusion(userName);
		realName = Utility.filteSqlInfusion(realName);
	 ...
	 * @throws DataException
	 */
	public List<Map<String, Object>> queryUserPhoneByGroupId(long groupId)
			throws Exception {
		Connection conn = MySQL.getConnection();
		try {
	 ...
	 * @throws DataException
	 */
	public List<Map<String, Object>> queryUserEmailByGroupId(long groupId)
			throws Exception {
		Connection conn = MySQL.getConnection();
		try {
	 ...
	 * @throws DataException
	 */
	public List<Map<String, Object>> queryGroupUsersByIds(String ids)
			throws Exception {
		Connection conn = MySQL.getConnection();
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryGroupById(Long groupId) throws Exception {
		Connection conn = MySQL.getConnection();

		try {
			Map<String, String> map = groupUserDao
					.queryGroupById(conn, groupId);
			List<Map<String, Object>> listUsers = groupUserDao
					.queryUsersByGroupId(conn, groupId);
	...
	public long deleteGroupUserByGroupId(long groupId) throws Exception {
		Connection conn = MySQL.getConnection();
		long result = -1;
		try {
			result = groupUserDao.deleteGroupUserByGroupId(conn, String
					.valueOf(groupId));
			if (result <= 0) {
				conn.rollback();
				return result;
	...
	public long addGroupUser(List<Long> userIds) throws Exception {
		Connection conn = MySQL.getConnection();
		long result = -1;
		try {
			int count = 0;
	...
	public long modifyGroup(long groupId, long adminId, List<Long> userIds,
			String groupName, String groupRemark, int cashStatus)
			throws Exception {
		Connection conn = MySQL.getConnection();
		long result = -1;
		try {
			result = groupDao.modifyGroup(conn, groupId, adminId, groupName,
			...
			if (userIds != null && userIds.size() > 0) {
				groupUserDao.deleteGroupUserByGroupId(conn, String
						.valueOf(groupId));
				for (Long userId : userIds) {
