...
public class FrontDownloadAction extends BasePageAction {
	...
	public String frontQueryDownlaodList() throws Exception{
		...
		String pageNum = Convert.strToStr(paramMap.get("pageNum"), null);
	...
	public String downloads() throws Exception{
		...
		try {
			...
			paramMap = downloadService.frontDownloadById(id);
				...
				String path = paramMap.get("attachment");
				String result=null;
				if(path!=null){
				...
				if(result.equals("success")){
					if(user == null ){
						operationLogService.addOperationLog("v_t_download_detail", "游客", IConstants.DOWNLOAD, "游客", 0, "用户下载资料", 1);
					}
					else{
