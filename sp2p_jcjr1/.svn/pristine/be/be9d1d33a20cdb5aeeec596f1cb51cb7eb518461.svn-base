...
public class MailBoxManagerAction extends BasePageAction {
	 ...
	 * @throws Exception
	 */
	public String queryMailBoxListPage() throws Exception {
		//Integer mailType = request.getInt("mailType", -1);
		Integer mailType = Convert.strToInt(paramMap.get("mailType"), -1);
		String beginTime = paramMap.get("beginTime");
		String endTime = paramMap.get("endTime");
		String senders = paramMap.get("sender");
		String sender = null;
		...
		try {
			...
			mailBoxManagerService.queryMailByCondition(pageBean, mailType,
					sender, beginTime, endTime);
	 ...
	 * @throws Exception
	 */
	public String queryMailBoxListUserPage() throws Exception {
		//Integer mailType = request.getInt("mailType", -1);
		Integer mailType = Convert.strToInt(paramMap.get("mailType"), -1);
		String beginTime = paramMap.get("beginTime");
		String endTime = paramMap.get("endTime");
	 ...
	 */
	private void changeLists2Lists(List<Map<String, Object>> lists)
		...
		try {
			...
			for (Map<String, Object> map : lists) {
				String username = "";
				mp = adminService.queryAdminById(Convert.strToLong(map
						.get("sender")
						+ "", -1));
		...
		try {
			...
			for (Map<String, Object> map : lists) {
				username = this.getUserNameById(Convert.strToLong(map
						...
						+ "", -1));
				if (username.equals("")) {
					username = this.getAdminNameById(Convert.strToLong(map
							.get("reciver")
							+ "", -1));
				}
	...
	public String addMail() throws IOException, DataException, SQLException {
		...
		try {
			...
			Admin user = (Admin) session().getAttribute(
			...
			result = homeInfoSettingService.addMail(id, receiverId, title,
					content, IConstants.MAIL_UN_READ, 2,
	 ...
	 * @throws Exception
	 */
	public String updateMailBoxInit() throws Exception {
		Long id = request.getLong("id", -1);
		try {
			paramMap = mailBoxManagerService.getMailById(id);
			// 把发件人Id换成发件人名称
	...
	public String updateMailBox() throws Exception {
		...
		String title = paramMap.get("mailTitle");
		String content = paramMap.get("mailContent");
		...
		try {
			...
			long result = mailBoxManagerService.updateMailBoxById(id, title,
					content);
			if (result > 0) {
				JSONUtils.printStr("1");
				return null;
	 ...
	 * @throws Exception 
	 */
	public String deleteMailBoxUser() throws Exception {
		String dynamicIds = request.getString("id");
	...
	public void setMailBoxManagerService(
			MailBoxManagerService mailBoxManagerService) {
