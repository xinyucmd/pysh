	...
	public String alipayReceive() throws Exception {
		...
		try {
			...
			String paynumber = URLDecoder.decode(appInfoMap.get("trade_no"),
					"utf-8");
			// 支付宝编号
			String notify_time = URLDecoder.decode(appInfoMap
					.get("notify_time"), "utf-8");// 支付宝编号
			...
			String buyer_email = URLDecoder.decode(appInfoMap
					.get("buyer_email"), "utf-8");// 支付银行
			...
			if (returnId < 0) {
				...
				sendMessage(userId, Convert
						.strToDouble(request("total_fee"), 0), 0);
				// ------------------
