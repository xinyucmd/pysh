...
<script type="text/javascript">
 $(function(){
   ...
   window.location.href=encodeURI(encodeURI("exportRechargeRecord.do?userName="+$("#userName").val()+"&&rechargeTime="+$("#startTime").val()+"&&rechargeType="+$("#rechargeType").val()+"&&statss="+$("#status").val()+"&&reEndTime="+$("#endTime").val()));
   });
 });
</script>
	<div>
					...
					<s:iterator value="pageBean.page" var="bean" status="s">
						<tr class="gvItem">
						  <%--  <td>
							...
							<s:if test="#bean.rechargeType==10">连连支付-手机</s:if>
							</td>
							<td>
								${bean.id}
							</td>
							...
							<td>
								<a href="javascript:show('${bean.ids }');">查看</a>
							  </td>
						</tr>
					</s:iterator>
					<tr>
