...
public class AwardService extends BaseService {
	...
	public int award1New(Connection conn, long userId,
		...
		if (relationLevel1Map != null) {
			...
			awardDao.addAward(conn, userId, level2UserId, level2Money,
					level1UserId, level1Money, investOrRepaymentId, type, 1,
	...
	// ----------------------奖励机制----------------------
	/***************************************************************************
	 * / 金额变动 @param conn @param userId用户编号(投资人和理财人) @param money变动金额 @param
	 * type1投资2还款 @param investOrRepaymentId投资/还款明细编号 @return -1,-2,-3,-4:
	 * 0：处理成功 1:此标的不属于实地认证标的，或机构担保标的。 2：当前用户已经期满1年，不需要进行奖励了。 3：此用户不是奖励体系的角色。
	 * 4：与上一级解除关系了不需要进行奖励。 @throws Exception
	 * 
	 ...
	 * select a.id as id, a.borrowId as borrowId,b.borrowWay as borrowWay from
	 * t_invest a left join t_borrow b on a.borrowId = b.id where a.id =
	 * "+investOrRepaymentId); }else if(type==IConstants.MONEY_TYPE_2){//还款
	 * dataSet = MySQL.executeQuery(conn, " select a.id as id, a.borrowId as
	 * borrowId,b.borrowWay as borrowWay from t_repayment a left join t_borrow b
	 * on a.borrowId = b.id where a.id = "+investOrRepaymentId); } Map<String,String>
	 * iorMap = BeanMapUtils.dataSetToMap(dataSet); //任何标的都产生提成 // int borrowWay =
	 * Convert.strToInt(iorMap.get("borrowWay"), -1); //
	 * if(borrowWay!=IConstants.BORROWWAY_TYPE_4&&borrowWay!=IConstants.BORROWWAY_TYPE_5){ //
	 * return 1; // } Map<String,String> userMap = userDao.queryUserById(conn,
	 * userId);//查询用户明细 SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd
	 * HH:mm:ss"); Date newDate = new Date(); Date endDate =
	 * sdf.parse(userMap.get("createTime"));
	 ...
	 * 3;//不在角色关系系统内， } Map<String,Object> map = new HashMap<String,
	 * Object>(); map = list.get(0); enable =
	 * Convert.strToInt(map.get("enable")+"",-1);//是否和上一级解除关系 parentId =
	 * Convert.strToLong(map.get("parentId")+"", -1);//投资人的上级是经纪人 } Map<String,Object>
	 * platformCostMap = getPlatformCost(); //奖励机制1：
	 * award1(conn,userId,userMap,relationMap,parentId,investOrRepaymentId,level,type,platformCostMap);
	 * 
	...
	public void monthAwardInfo(Connection conn, String table, String condition,
			...
			throws Exception {
		table = Utility.filteSqlInfusion(table);
		condition = Utility.filteSqlInfusion(condition);
		...
		for (Map<String, Object> map : list) {
			...
			awardDao.addAward(conn, userId, level2userId, level2money,
					level1userId, level1money, -1, -1, 1, xmin,
	...
	public void queryAwardLevel2(Long level1userId, Long level2userId,
			...
			throws Exception {
		level2userName = Utility.filteSqlInfusion(level2userName);
	...
	public void queryIoRRepayInfo(Long userId, Long level2userId,
			PageBean<Map<String, Object>> pageBean) throws Exception {
		StringBuffer condition = new StringBuffer();
		if (userId != null && userId > 0) {
			condition.append(" AND userId = " + userId);
		...
		Connection conn = MySQL.getConnection();
		try {
			dataPage(conn, pageBean, " v_t_award_ior_repayinfo ", " * ", "  ",
					condition.toString());
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public void queryAwardLevel1(Long level1userId, String level1userName,
		...
		level1userName = Utility.filteSqlInfusion(level1userName);
		level2userName = Utility.filteSqlInfusion(level2userName);
	...
	public Map<String, String> queryIorMoneys(Long level2userId, Long userId,String type)
			throws Exception {
		StringBuffer condition = new StringBuffer();
		if (level2userId != null && level2userId > 0) {
			condition.append(" AND level2userId = " + level2userId);
		...
		try {
			if(type.equals("2")){
				dataSet = MySQL.executeQuery(conn,
						" select sum(level2money) as moneys from v_t_award_ior_repayinfo where 1=1 "
								+ condition.toString());
			}
			else{
			dataSet = MySQL.executeQuery(conn,
	...
	public Map<String, String> queryAwardTSum(long leve1userId, String username)
		...
		try {
			...
			DataSet ds = MySQL.executeQuery(conn,
					" select round(sum(level1money),2) as level1moneySum from  v_t_award_leve1  where 1=1 "
							+ condition + "");
			return BeanMapUtils.dataSetToMap(ds);
		} catch (Exception e) {
			log.error(e);
	...
	public void queryLeve2SumCount(long leve2userId, String leve2Name,
			PageBean<Map<String, Object>> pageBean) throws Exception {
		leve2Name = Utility.filteSqlInfusion(leve2Name);
	...
	public Map<String, String> queryLeve2CountToMap(long leve2userId,
		...
		try {
			DataSet dataSet = MySQL
					.executeQuery(
							conn,
							" select round(sum(level2money),2) as sumLeve2Money from v_t_award_leve2_sum_money where 1=1 "
									+ condition.toString());
	...
	public Map<String, String> queryLevel1Sum(Long level1userId,
		...
		try {
			DataSet dataSet = MySQL.executeQuery(conn,
					" select round(sum(level1money),2) as level1money from v_t_award_detail where 1=1 "
							+ condition.toString());
	 ...
	 public double changeDateToStr(PageBean<Map<String, Object>> pageBean){
		 List<Map<String, Object>> list = pageBean.getPage();
		     double countMoney = 0;
			...
			for (Map<String, Object> map : list) {
				...
				countMoney = countMoney
						+ Convert.strToDouble(map.get("level2moneys") + "", 0);
			}
			return countMoney;
	 }
