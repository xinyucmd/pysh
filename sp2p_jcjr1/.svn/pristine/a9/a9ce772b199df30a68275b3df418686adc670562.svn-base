...
public class DebtAppAction extends BaseAppAction {
	...
	public String queryFrontAllDebt() throws IOException {
		...
		try {
			...
			assignmentDebtService.queryAllDebt(borrowTitle,debtSum, isLate, publishDays, "1,2", pageBean);
	...
	public String queryFrontSuccessDebt() throws IOException {
		...
		try {
			...
			long publishDays = Convert.strToLong(appInfoMap.get("publishDays"), -1);
			String borrowTitle = appInfoMap.get("borrowTitle");
			assignmentDebtService.queryAllDebt(borrowTitle,debtSum, isLate, publishDays, "2", pageBean);
			jsonMap.put("pageBean", pageBean);
			jsonMap.put("error", "-1");
			jsonMap.put("msg", "成功");
			JSONUtils.printObject(jsonMap);
	...
	public String queryDebtDetail() throws IOException {
		...
		try {
			...
			if (map != null) {
				...
				if((paramMap.get("debtStatus")).equals("3")){
					paramMap.put("remainDays", DateUtil.remainDateToString(new Date(),DateUtil.dateAddDay(new Date(),0)));
				}
	...
	public String addAuctingDebt() throws IOException {
		...
		try {
			...
			if (null != m) {
				...
				if(isApplyPro==2&&pwd.equals(del)){
					jsonMap.put("error", "-3");
			...
			if (debtMap != null && userMap != null) {
				...
				String pwd=com.shove.security.Encrypt.decrypt3DES(appInfoMap.get("pwd"), IConstants.PASS_KEY);
				pwd = Encrypt.MD5(pwd+IConstants.PASS_KEY).toLowerCase();
				...
				paramMap.put("userName", userMap.get("username"));
