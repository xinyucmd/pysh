...
<body>
 ...
 var realpercentage=[];    //销量数组（实际用来盛放Y坐标值）
 var grouppercentage=[];    //销量数组（实际用来盛放Y坐标值）
 $.ajax({
 type : "post",
 async : true,            //异步请求（同步请求将会锁住浏览器，用户其他操作必须等待请求完成才可以执行）
 url : "borrowCharts.do",    //请求发送到TestServlet处
 data : {},
 dataType : "json", 
 //返回数据形式为json
 success : function(result) {
     ...
     if (result) {
            ...
            for(var i=0;i<result.length;i++){   
                    	realpercentage.push(Math.round(result[i].realInvest/(result[i].groupInvest+result[i].realInvest)*100));
                        grouppercentage.push(Math.round(result[i].groupInvest/(result[i].groupInvest+result[i].realInvest)*100));
                }
            ...
            myChart.setOption({        //加载数据图表
                ...
                series: [{
                	...
                	// 根据名字对应到相应的系列
                    name: '机构用户投标占比',
                    data: grouppercentage,
                    formatter:'{grouppercentage}%',
                    itemStyle:{
                        normal:{color:'#B5C334'}
                    }
                }]
