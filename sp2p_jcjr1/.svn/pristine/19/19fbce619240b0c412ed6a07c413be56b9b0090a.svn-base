...
public class AfterCreditManageService extends BaseService {
	...
	public Map<String, String> queryUserById(long id) throws Exception {
		...
		try {
			map = afterCreditManageDao.queryUserById(conn, id);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public void queryLastRepayMentByCondition(String userName, int borrowWay,
			String realName, String title, String timeStart, String timeEnd,
	...
	public Map<String, String> queryRepaymentAmount(String userName,
		...
		try {
			map = afterCreditManageDao.queryRepaymentAmount(conn, userName,
					borrowWay, realName, title, status, timeStart, timeEnd);
			conn.commit();
