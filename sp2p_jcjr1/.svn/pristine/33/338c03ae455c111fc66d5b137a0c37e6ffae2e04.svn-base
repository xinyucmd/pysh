...
public class UploadUtil {
	 ...
	 * @throws Exception
	 */
	public static void uploadByFile(File file, String source, String fileName)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	public static String getByAllParams(Files file, FileCommon fileCommon,
			String realpath,String type, String cp) throws Exception {
		...
		if (!"GIF,TIF,JPG,JPEG,PNG,BMP,DOC,DOCX,WPS,PPT,PPTX,DPS,XLS,XLSX,ET,TXT,PDF,RAR,ZIP".contains(getExt.toUpperCase()) && "img".equals(cp)) {
			return "文件类型不对!";
			}else if("GIF,TIF,JPG,JPEG,PNG,BMP".contains(getExt.toUpperCase()) && "doc".equals(cp)){
			return "文件类型不对!";
			}
		File f = file.getFiles();
		if (f.exists()) {
			String filetype2 = VerifyTruePicture.getFileByFile(f,cp);
		...
		// 若指定了大小限制，则进行检查
		double fileLimitSize = Double
				.parseDouble(fileCommon.getFileLimitSize());
		if (fileLimitSize > 0) {
			long getFileSize = 0L;
			if (fileCommon.getSizeUnit().equalsIgnoreCase("M")) {
				getFileSize = new Double(fileLimitSize * 1024 * 1024)
						.longValue();
			}
	 ...
	 * @throws Exception
	 */
	public static String uploadByFileapp(Files file, String source)
			throws Exception {
		FileInputStream fis = null;
