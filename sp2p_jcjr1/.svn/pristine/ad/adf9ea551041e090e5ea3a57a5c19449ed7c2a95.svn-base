...
public class MyFinanceAction extends BaseAppAction {
	 ...
	 */
     public String queryTyjInvestDetail(){
		...
		try {
			...
			/**查询用户体验金*/
			List<Map<String, Object>> tyjList = new ArrayList<Map<String, Object>>();
			tyjList = financeService.queryUserSendTyj(userId);
			...
			double  rate = Convert.strToDouble(borrowMap.get("rate"), 0);
			int  day = Convert.strToInt(borrowMap.get("day"), 0);
			double  rates = 8888*rate/100f*day/365; 
			...
			if(list!=null && list.size()>0){
				invest_sum = list.size();
			}
			jsonMap.put("tyjList", tyjList);
			jsonMap.put("borrowMap", borrowMap);
			jsonMap.put("invest_sum", invest_sum);
			jsonMap.put("rates", rates);//收益
      ...
      * @return
      */
     public String queryTyjInvestDetails(){
     	
     	Map<String, String> infoMap = this.getAppInfoMap();
 		long borrowId = Convert.strToLong(infoMap.get("borrowId"), 0); 
 		pageBean.setPageNum(infoMap.get("currPage"));
 	 ...
 	 */
 	 public String addTyjInvest(){
 			...
 			try {
 				if(userId<0){
 					jsonMap.put("msg", "您未登录");//未登陆
 					JSONUtils.printObject(jsonMap);
 					return null;
 				}
 				
 				if(borrowId<=0){
 					jsonMap.put("msg", "参数错误");//未登陆
 					JSONUtils.printObject(jsonMap);
 					return null;
 				}
	...
	public String financeList() throws Exception {
		...
		try {
			...
			if(!"android".equals(flag)){
				...
				List<Map<String, Object>> list = financeService.queryBorrowAll(borrow_t,borrowStatus, borrowWay, title,
						paymentMode, purpose, deadline, reward, arStart, arEnd,
						IConstants.SORT_TYPE_DESC);
	...
	public String index() throws SQLException, DataException, IOException {
		...
		try {
			...
			mapList = financeService.queryLastestBorrowApp();
			if(mapList == null || mapList.size() <= 0){
				mapList = financeService.queryLastestBorrowAppByBorrowWay();
			}
			jsonMap.put("mapList", mapList);
			//图片滚动
			pageBean.setPageSize(50);
			publicModelService.queryBannerListPage(pageBean,1);
		...
		try {
			...
			if(userId > -1){
				...
				if(userMap != null && userMap.size() > 0){
					usableSum = userMap.get("usableSum");
				}
			}else{
				//request().setAttribute("isLogin", 0);
				jsonMap.put("isLogin", 0);
			}
			...
			if (!"".equals(idStr) && StringUtils.isNumericSpace(idStr)) {
				...
				if (borrowDetailMap != null && borrowDetailMap.size() > 0) {
					...
					if("6".equals(borrowWays)){
						 jsonMap.put("bonus_type", "");
					}
					//红包
					jsonMap.put("bonus_type", "");
					if(userId > -1){
						...
						if(reg>=0){
							 hbMap = financeService.queryUserHbMoney(userId,1);
							 //request().setAttribute("bonus_type", 1);
							 jsonMap.put("bonus_type", 1);
						}
						if(invest>=0){
							 hbMap = financeService.queryUserHbMoney(userId,2);
							 //request().setAttribute("bonus_type", 2);
							 jsonMap.put("bonus_type", 2);
						}
						if(recomm>=0){
							 hbMap = financeService.queryUserHbMoney(userId,3);
							 //request().setAttribute("bonus_type", 3);
							 jsonMap.put("bonus_type", 3);
						}
						if(reg>=0 && invest>=0){
							 hbMap = financeService.queryUserHbMoney(userId,12);
							 //request().setAttribute("bonus_type", 12);
							 jsonMap.put("bonus_type", 12);
						}
						if(reg>=0 && recomm>=0){
							 hbMap = financeService.queryUserHbMoney(userId,13);
							 //request().setAttribute("bonus_type", 13);
							 jsonMap.put("bonus_type", 13);
						}
						if(invest>=0 && recomm>=0){
							 hbMap = financeService.queryUserHbMoney(userId,23);
							 //request().setAttribute("bonus_type", 23);
							 jsonMap.put("bonus_type", 23);
						}
						
						if(reg>=0 && invest>=0 && recomm>=0){
							 hbMap = financeService.queryUserHbMoney(userId,123);
							 request().setAttribute("bonus_type", 123);//三 代表 所有红包之和
							 jsonMap.put("bonus_type", 123);
						}
						
						if(hbMap!=null && hbMap.size()>0){
							//request().setAttribute("hbMap", hbMap);
							jsonMap.put("hbMap", hbMap);
						}else{
						...
						jsonMap.put("hbConfigMap", hbConfigMap);
						
						
						Map<String,String> map_6_24 = financeService.queryBonus_6_24_sum(userId);
						if(map_6_24!=null && map_6_24.size()>0){
							String bonus_able_6_24 = map_6_24.get("bonus_able");
							jsonMap.put("bonus_able_6_24", bonus_able_6_24);
						}else{
							jsonMap.put("bonus_able_6_24", "0");
						}
						
						 
					}
					
					jsonMap.put("borrowWays", borrowWays);
					...
					Map<String, String> feestateMap = (Map<String, String>) JSONObject
							.toBean(JSONObject.fromObject(feestate),
									HashMap.class);
					// --end
					String nid_log = borrowDetailMap.get("nid_log");
					Map<String, String> person = userService
							.queryPersonById(Convert.strToLong(borrowDetailMap
									.get("publisher"), -1l));
					paramMap.put("nativePlacePro", "-1");
					paramMap.put("nativePlaceCity", "-1");
					if (person != null && person.size() > 0) {
						paramMap.put("nativePlacePro", person
								.get("nativePlacePro"));
						paramMap.put("nativePlaceCity",person
								.get("nativePlaceCity"));
					}
					...
					double annualRate = Convert.strToDouble(borrowDetailMap
							.get("annualRate")
							+ "", 0);
					int deadline = Convert.strToInt(borrowDetailMap
							.get("deadline")
							+ "", 0);
					int paymentMode = Convert.strToInt(borrowDetailMap
							.get("paymentMode")
							+ "", -1);
					int isDayThe = Convert.strToInt(borrowDetailMap
							.get("isDayThe")
							+ "", 1);
					 ...
					 * .get(IAmountConstants.INVEST_FEE_RATE) + "", 0);
					 */
					...
					if(hasPwd ==1 && addInterest>0){
						addInterest = 0;
						borrowDetailMap.put("add_interest", "0");
					}
					annualRate = annualRate + addInterest;
					if (paymentMode == 1) {
						// 按月等额还款
						earnMap = au.earnCalculateMonth(investAmount,
								borrowSum, annualRate, deadline, 0, isDayThe,
								2, costFee);
						earnAmount = earnMap.get("msg") + "";
						realAmount = earnMap.get("realAmount");
						totalInterest = earnMap.get("totalInterest");
					...
					borrowDetailMap.put("realAmount", realAmount);
					borrowDetailMap.put("totalInterest", totalInterest);
...
//					else if (borrowWay.equals(IConstants.BORROW_TYPE_INSTITUTION_FLOW)) {
//						jsonMap.put("borrowWay",IConstants.BORROW_TYPE_INSTITUTION_FLOW_STR);
//					}
					// --------------------
					...
					// request().setAttribute("borrowDetailMap",
					// borrowDetailMap);
					// 借款人资料
					...
					//加入投资人可用余额
					borrowUserMap.put("usableSum", usableSum);
					...
					// request().setAttribute("borrowRecordMap",
					// borrowRecordMap);
					// -----------add by houli
					String wStatus = judgeStatus(Convert
							.strToInt(borrowWay, -1), Convert.strToLong(
							borrowDetailMap.get("publisher"), -100));
					if (wStatus == null) {
					...
					//whb 适配2015-01-28版本 借款说明
					if(StringUtils.isBlank(borrowDetailMap.get("businessDetail").trim()) || "null".equals(borrowDetailMap.get("businessDetail").trim())){
						borrowDetailMap.put("businessDetail", borrowDetailMap.get("borrowInfo"));
					}
	...
	public String financeInvest() throws Exception {
		...
		try {
			...
			if (!dealPwd.equals(m.get("dealpwd"))) {
...
//			}
			...
			if ("1".equals(hasPWD)) {
				String investPWD = appInfoMap.get("investPWD") == null ? ""
						: appInfoMap.get("investPWD");
				if (StringUtils.isBlank(investPWD)) {
					jsonMap.put("error", "3");
					jsonMap.put("msg", "投标密码不能为空");
					JSONUtils.printObject(jsonMap);
					return null;
				}
				Map<String, String> investPWDMap = financeService.getInvestPWD(
			...
			int num = 0;
			if (status == 1) {
				double smallestFlowUnit = Convert.strToDouble(appInfoMap
						.get("smallestFlowUnit"), 0.0);
				...
				String result = appInfoMap.get("result");
				...
				Map<String, String> borrowDetailMap = financeService.queryBorrowDetailById(idLong);
				Map<String, String> investMaps = financeService.queryInvest(user.getId(), idLong);
				Map<String, String> borrowMap = financeService.queryBorrow(idLong);
				double maxMoneyValue = Convert.strToDouble(borrowDetailMap.get("maxMoneyValue"), 0);
				double hasInvestAmount = Convert.strToDouble(investMaps.get("investAmount"), 0);
			
				 
				double results  = maxMoneyValue - hasInvestAmount;//投资上限减去已经投资金额==可投金额
				if(results<=0){
					jsonMap.put("msg", "累计投资已达上线");
					JSONUtils.printObject(jsonMap);
					return null;
				}else{
					int fs = (int) (results/smallestFlowUnit);
					if(fs-num<0){
						jsonMap.put("msg", "最多可投"+fs+"份");
						JSONUtils.printObject(jsonMap);
						return null;
				    }
				}
			...
			int isUseHb = Convert.strToInt(appInfoMap.get("isUseHb"), 0);//是否使用红包    0-不使用，1-使用
			double bonus_avaliable = Convert.strToDouble(appInfoMap.get("bonus_avaliable"), 0);//可用红包金额
			int bonus_type = Convert.strToInt(appInfoMap.get("bonus_type"), 0);//类型
			int isUseHb_6_24 = Convert.strToInt(appInfoMap.get("isUseHb_6_24"), 0);//是否使用红包    0-不使用，1-使用
			...
			Map<String, String> result = financeService.addBorrowInvest(idLong,
					user.getId(), "", Math.floor(amountDouble), getBasePath(), user
							.getUserName(), status, num,isUseHb,bonus_avaliable,bonus_type,isUseHb_6_24);
			...
			if ("".equals(result.get("ret_desc"))) {
				...
				jsonMap.put("msg", "投标失败["+result.get("ret_desc")+"]");
				JSONUtils.printObject(jsonMap);
			}
		} catch (Exception e) {
			jsonMap.put("error", "9");
