 ...
 */
	...
	public Long UpdateFinnceRating(Connection conn,Long id, int score, int prescore)
			throws Exception {
		Long resultId = -1L;
		...
		Integer type = IConstants.USER_INTERGRALTYPEVIP;
			...
			Map<String, String> map = userIntegralDao.queryUserIntegral2(conn,
					id, 2, intergraltype);
			if (map == null) {
				resultId = userManageDao.addserintegraldetail(conn, id, score,
	...
	public Long UpdateLoginRating(Long id, int score, int prescore,
		...
		try {
			if (loignCount <= 2) {
				...
				if (map == null) {
					resultId = userManageDao.addserintegraldetail(conn, id,
							score, intergraltype, type, remark, changetype);
				} else {
