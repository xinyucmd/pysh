...
public class AdminDao {
	 ...
	 * @throws SQLException
	 */
	public Long addAdmin(Connection conn, String userName, String password,
			int enable, long roleId, String realName, String telphone,
			String qq, String email, String img, String isLeader,Integer sex,String card,String summary,Integer nativePlacePro,Integer nativePlaceCity,String address)
			throws SQLException {
	 ...
	 * @throws SQLException
	 */
	public Long updateAdmin(Connection conn, long adminId, String password,
			Integer enable, String lastIP, Long roleId, String realName,
			String telphone, String qq, String email, String img,
			String isLeader,Integer sex,String card,String summary,Integer nativePlacePro,Integer nativePlaceCity,String address) throws SQLException {
	 ...
	 * @throws SQLException
	 */
	public void deleteAdmins(Connection conn, String adminIds)
			throws SQLException {
		adminIds=com.shove.web.Utility.filteSqlInfusion(adminIds);
		...
		Dao.Tables.t_admin t_admin = new Dao().new Tables().new t_admin();
		t_admin.delete(conn, " id in (" + idSQL
				+ ")");// 对管理员进行删除，删除条数返回到returnId上面
	}
	 ...
	 */
	public List<Map<String, Object>> queryAdminList(Connection conn,
			String userName, Integer enable) throws SQLException, DataException {
		...
		Dao.Tables.t_admin t_admin = new Dao().new Tables().new t_admin();
		DataSet ds = t_admin.open(conn, "", condition.toString(), " id DESC  ",
				-1, -1);
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryAdminByNamePass(Connection conn,
		...
		condition.append(" AND password = '"
				+ StringEscapeUtils.escapeSql(com.shove.security.Encrypt
						.MD5(password.trim())) + "'");
		Dao.Tables.t_admin t_admin = new Dao().new Tables().new t_admin();
	...
	public Map<String, String> queryIdByUser(Connection conn, String userName)
		...
		userName=com.shove.web.Utility.filteSqlInfusion(userName);
		Dao.Tables.t_admin admin = new Dao().new Tables().new t_admin();
		DataSet dataSet = admin.open(conn, "id",
				" userName='" + StringEscapeUtils.escapeSql(userName.trim()) + "'", "", -1, -1);
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryAdminById(Connection conn, long adminId)
			throws SQLException, DataException {
	...
	public Map<String, String> queryAllAdmin(Connection conn) throws Exception, DataException {
		Dao.Tables.t_admin admin = new Dao().new Tables().new t_admin();
		DataSet dataSet = admin.open(conn, "count(id) as num",
				"", "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	}

	public Map<String, String> queryAdmin(Connection conn) throws Exception, DataException {
		Dao.Tables.t_admin admin = new Dao().new Tables().new t_admin();
		DataSet dataSet = admin.open(conn, "isFirstLogin",
				"id = -1", "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	}

	public Map<String, String> queryAdminByName(Connection conn, String userName) throws Exception {
		Dao.Tables.t_admin admin = new Dao().new Tables().new t_admin();
		DataSet dataSet = admin.open(conn, "*",
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryIdByInfo(Connection conn, String userName,String password) throws Exception {
		Dao.Tables.t_admin admin = new Dao().new Tables().new t_admin();
		DataSet dataSet = admin.open(conn, "id",
				"userName = '" + userName+"'"+"and password = '"+ password+"'", "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	 ...
	 * @throws DataException
	 */
	public Map<String,String>  queryUtool(Connection conn) 
	throws SQLException, DataException{
Dao.Tables.t_utool utool = new Dao().new Tables().new t_utool();
DataSet dataSet = utool.open(conn, "*", "",
		"", -1, -1);
return BeanMapUtils.dataSetToMap(dataSet);	
}

	public long updateAdminPass(Connection conn, String password) throws SQLException{
			Dao.Tables.t_admin admin = new Dao().new Tables().new t_admin();
			 admin.password.setValue(password);
			 admin.isFirstLogin.setValue(2);
			 admin.loginErrorCount.setValue(0);
			 return admin.update(conn, "id = -1");
		}
	 ...
	 * @throws SQLException
	 */
	public long resetAdminState(Connection conn,int loginErrorCount,int isLoginLimit,long userId) throws SQLException {
		Dao.Tables.t_admin  admin = new Dao().new Tables().new t_admin();
		admin.loginErrorCount.setValue(loginErrorCount);
		admin.isLoginLimit.setValue(isLoginLimit);
		return admin.update(conn, " id = "+ userId);
	}
	/**
	 * 设置用户登录错误次数和登陆时间
	 * @param conn
	 ...
	 * @throws SQLException
	 */
	public long updateAdminState(Connection conn,int loginErrorCount,String lastDate,long userId) throws SQLException {
		Dao.Tables.t_admin  admin = new Dao().new Tables().new t_admin();
		admin.loginErrorCount.setValue(loginErrorCount);
		admin.lastTime.setValue(lastDate);
		return admin.update(conn, " id = "+ userId);
	}
