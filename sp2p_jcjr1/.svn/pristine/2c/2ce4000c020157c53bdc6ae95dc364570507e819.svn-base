...
//		request().setAttribute("inverseRecorde", inverseRecorde);
	...
	public String queryUserFriendList() throws SQLException, Exception {
		...
		if (lists != null) {
			for (Map<String, Object> map : lists) {
				...
				String encUserId = ds.encrypt(userId);
				map.put("encUserId", encUserId);
				
				String tcmoduleId = map.get("tcmoduleId")+"";
				String enTcmoduleId = ds.encrypt(tcmoduleId);
				map.put("enTcmoduleId", enTcmoduleId);
			}
		}
	...
	public String deleteUserFriend()throws SQLException,DataException,Exception{
		DesSecurityUtil ds =new DesSecurityUtil();
		String enuId = paramMap.get("userId");
		String enaId = paramMap.get("attentionId");
		String enuserId = ds.decrypt(enuId);
		String enattentionId = ds.decrypt(enaId);
		Long userId=Convert.strToLong(enuserId, -1L);
		Long attentionUserId=Convert.strToLong(enattentionId,-1L);
		guaranteeService.DeteleUserFriends(attentionUserId, userId);
		paramMap.put("attention", "attention");
		paramMap.put("id", enuId);
