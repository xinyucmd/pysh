...
public class ApplicationListener extends ContextLoaderListener implements
	...
	public void contextInitialized(ServletContextEvent sce) {
		...
		// 初始化平台收费
		PlatformCostService platformCostService = (PlatformCostService) webApplicationContext
				.getBean("platformCostService");
		List<Map<String, Object>> platformCostList = null;

		// 初始化通知模板
		ShoveApproveNoticeTemplateService approveNoticeTemplateService = (ShoveApproveNoticeTemplateService) webApplicationContext
				.getBean("shoveApproveNoticeTemplateService");
		...
		try {
			...
			for (Map<String, Object> platformCostMap : platformCostList) {
				...
				int costMode = Convert.strToInt(platformCostMap.get("costMode")
						+ "", 0);
				if (costMode == 1) {
					feeApplicationMap.put(platformCostMap.get("alias") + "",
							costFee * 0.01);
				} else {
					feeApplicationMap.put(platformCostMap.get("alias") + "",
							costFee);
			...
			// 模板
			noticeTemplateList = approveNoticeTemplateService
					.queryAllInformTemplate();
			for (Map<String, Object> noticeTemplateMap : noticeTemplateList) {
				informTemplateApplicationMap.put(noticeTemplateMap.get("nid")
						+ "", noticeTemplateMap.get("template") + "");
				noticeTemplateMap = null;
			}
			// //初始化邮件参数
			GetMailMsgOnUpService getMailMsgOnUpService = (GetMailMsgOnUpService) webApplicationContext
					.getBean("getMailMsgOnUpService");
			...
			if (mailSet != null) {
				...
				IConstants.NICK = javax.mail.internet.MimeUtility
						.encodeText(mailSet.get("sendname"))
						+ "";
				IConstants.MAIL_PORT = Convert.strToInt(mailSet.get("port"), 25);
			}
			// //初始化SEO参数
			GetSEOConfigOnUpService getSEOConfigOnUpService = (GetSEOConfigOnUpService) webApplicationContext
					.getBean("getSEOConfigOnUpService");
			Map<String, String> seoConfig = getSEOConfigOnUpService
					.getSEOConfig();
			if (seoConfig != null) {
			...
			List<Map<String, Object>> seoConfig1 = getSEOConfigOnUpService
					.getRegistCode();
			for (Map<String, Object> map : seoConfig1) {
				int id = Convert.strToInt(map.get("id") + "", 0);
				if (id == 3) {
					IConstants.SERIAL_NUMBER = map.get("value") + "";
				} else if (id == 4) {
					IConstants.SERIAL_KEY = map.get("value") + "";
				}
			}

			// 初始化 支付方式
			FundManagementService fundManagementService = (FundManagementService) webApplicationContext
					.getBean("fundManagementService");
			List<Map<String, Object>> mapList = fundManagementService
					.queryAccountPaymentList();
			for (Map<String, Object> map : mapList) {
				...
				if ("gopay".equals(nid)) {
					GopayConfig.gopay_virCardNoIn = maps.get("virCardNoIn")
							+ "";
					GopayConfig.gopay_verficationCode = maps
							.get("VerficationCode")
							+ "";
					GopayConfig.gopay_merchantID = maps.get("merchantID") + "";
				}
			}
			FrontMyPaymentService frontMyPayment = (FrontMyPaymentService) webApplicationContext
					.getBean("frontpayService");
			...
			System.gc();
			context.setAttribute(IAmountConstants.FEE_APPLICATION,
					feeApplicationMap);
			context.setAttribute(
					IInformTemplateConstants.INFORM_TEMPLATE_APPLICATION,
					informTemplateApplicationMap);
		}
