...
public class InvestDao {
	...
	*/
	 ...
	 * @throws Exception
	 */
	public Map<String, String> queryUsreType(Connection conn,
			long borrowId ) throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		command.append(" select a.id,a.borrowAmount,a.borrowTitle,a.publishTime,a.borrowWay,a.bonding_id,b.adminId,b.userId from t_borrow a ,t_admin_user b where a.publisher = b.userId and a.id = "+borrowId );
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		command = null;
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	...
	public Map<String, String> queryLoanCompany(Connection conn, long adminId)
	  ...
	  * @throws DataException
	  */
	public Map<String, String> queryBondingCompany(Connection conn, long adminId)
			throws SQLException, DataException {
		StringBuffer sql = new StringBuffer();
		sql.append(" select b.id as loan_bonding_id, b.model_type as model_type,b.desc as loan_bonding_desc,b.bonding_letter_path as bonding_company_letter_path, "
				 ...
				 + " from t_company_loan a,t_company_loan_bonding b,t_company_bonding c where a.id = b.loan_comp_id and b.bonding_comp_id = c.id and a.user_id = "+ adminId );
		DataSet dataSet = MySQL.executeQuery(conn, sql.toString());
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	 ...
	 * @throws SQLException 
	 */
	public List<Map<String, Object>> queryInvestMomey(Connection  conn,long invest_id,long borrowId,long userId) throws SQLException, DataException{
		StringBuffer command = new StringBuffer();
		...
		if (userId>0) {
			command.append(" and u.id = "+userId );
		}
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		dataSet.tables.get(0).rows.genRowsMap();
		command = null;
	 ...
	 * @throws SQLException 
	 */
	public List<Map<String,Object>> queryInvestMomeyOld(Connection  conn,long invest_id,long borrowId) throws SQLException, DataException{
		StringBuffer command = new StringBuffer();
		command.append(" SELECT u.username as username,  round(sum(b.recivedInterest),2) AS recivedInterest, round(SUM(b.recivedPrincipal),2) AS recivedPrincipal, round(sum(b.recivedInterest +b.recivedPrincipal ),2)  as sumPI, round((a.stillInterest + a.stillPrincipal),2) as stillPI ,date_format(tb.auditTime,'%Y-%m-%d') as starTime,CASE tb.isDayThe WHEN 1 THEN   date_format(DATE_SUB(tb.auditTime , INTERVAL -tb.deadline MONTH),'%Y-%m-%d')   WHEN 2 THEN date_format(DATE_SUB(tb.auditTime , INTERVAL -tb.deadline DAY),'%Y-%m-%d')   END  as endTime , tb.isDayThe as isDayThe ,tb.annualRate as annualRate ,tb.deadline as deadline ");
		command.append(" FROM  t_repayment a LEFT JOIN t_invest_repayment b  ON a.id = b.repayId   LEFT JOIN t_user u  on u.id = b.`owner` LEFT JOIN  t_borrow  tb on tb.id = a.borrowId WHERE  a.borrowId ="+borrowId+"  " );
		if (invest_id>0) {
	 ...
	 * @throws SQLException 
	 */
	public List<Map<String,Object>>  queryUsername(Connection  conn, long userId ,long invest_id) throws SQLException, DataException{
		
		 StringBuffer command = new StringBuffer();
		command.append(" SELECT u.username as username,f.realName as realName,f.idNO as idNo,f.cellPhone as cellPhone FROM t_user u LEFT JOIN t_person f ON f.userId =u.id WHERE u.id = "+userId);
		if (invest_id>0) {
	 ...
	 * @throws SQLException 
	 */
	public Map<String,String>  queryUsernameNow(Connection conn, long userId) throws SQLException, DataException{
	...
	*/
	 ...
	 * @throws SQLException
	 */
	public long updateAnnualRateDebtBDDouble(Connection conn,double annualRateDebtBDDouble ,long invest_id) throws SQLException{
		String command = "update t_invest  set annualRateDebtBDDouble = "+annualRateDebtBDDouble+" where id =  "+invest_id;
		return MySQL.executeNonQuery(conn, command);
	}
