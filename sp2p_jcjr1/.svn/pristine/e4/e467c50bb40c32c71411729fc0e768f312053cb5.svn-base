	...
	<head>
	    ...
	    $(function(){
		    ...
		    $("#excel").click(function(){
				 $("#excel").attr("disabled",true);
				    window.location.href=encodeURI(encodeURI("exportLockedUsersInfo.do?userName="+$("#userName").val()+"&&realName="+$("#realName").val()+"&&startTime="+$("#startTime").val()+"&&endTime="+$("#endTime").val()));
	   ...
	   function initListInfo(praData) {
		    ...
		    param["paramMap.endTime"] = $("#endTime").val();
	   		$.shovePost("queryLockedUsersInfo.do",praData,initCallBack);
   		}
   		...
   		function deleteById(ids){
	 		if(window.confirm("确定要启用吗?")){
	 			window.location.href = "unLockedUsers.do?id="+ids;
	 		}
   		}
					...
					<table  border="0" cellspacing="0" cellpadding="0">
							...
							<td width="100" height="28"  class="main_alll_h2">
								...
								<td width="100" height="28"  class="xxk_all_a">
									<a href="queryLockingUsersInit.do">添加锁定用户列表</a>
								</td>
								<td>
									&nbsp;
								</td>
						...
						<tbody>
							<tr>
								<td class="f66" align="left" width="50%" height="36px">
									...
									&nbsp;
									锁定时间:<s:textfield id="startTime" onclick="WdatePicker()" readonly="true" name="startTime" theme="simple" ></s:textfield>
										--
