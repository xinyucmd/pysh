...
#ul_div_check li{ line-height:40px;}
</style>
<body>
<!-- 引用头部公共部分 -->
        ...
        <div>
        	 <div>
        	     <ul>
        	      ...
        	      <li>
                           ...
                           <input type="button" value="新增" class="theme-login"/>
                           <input type="button" value="添加" class="theme-login1"/>
        	      </li>
        	      <li>
             	  ...
             	  <ul id="ul_input" style="position:absolute; display:none; background:#fff; border:solid 3px #c7c7c7; border-radius:10px; width:600px; height:auto; padding:20px; left:20%; top:50px;">
                    ...
                    <span  style="color: red;font-size: 12px"class="formtips">新增合作机构旗下的借款用户</span>
                    </li>
                    ...
                    <li>
                       <span class="s_login_txt">图片验证码：</span>
                	...
                	<input id="btn_cellregs" type="button" value="   注      册    "  onclick="reg_Buton()" class="input_c"/>
                	</li>
            ...
            <div id="ul_div_check" style="position:absolute; display:none; background:#fff; border:solid 3px #c7c7c7; border-radius:10px; width:600px; height:auto; padding:20px; left:20%; top:50px;"> 
                 <ul>
                    ...
                    <li>
                    <span class="s_login_txt" style="color: red">*</span>
                    <span  style="color: red;font-size: 12px"class="formtips">将系统已有用户变更为合作机构旗下的借款用户</span>
                    </li>
                    <li>
                    <span class="s_login_txt">用户名：</span>
                    <input name="paramMap.userName"  id="userName_check" type="text" class="input_a"/>
                    <span  style="color: red" id="s_userName_check" class="formtips"></span>
                    </li>
                    
                    <li>
                    <span class="s_login_txt">密码：</span>
                    <input name="paramMap.password" id="password_check" type="password" class="input_a" />
                    <span  style="color: red" id="s_password_check" class="formtips"></span>
                    </li>
                     
                    <li>
                       <span class="s_login_txt">用户类型：</span>
                       <select id="isFlag_check">
                          <option value="0">普通用户</option>
                          <option value="1">高级用户</option>
                       </select>
                       <span style="color: red;font-size: 12px" id="" class="formtips onError">标注：普通用户只能发布单个债权，高级用户可以发布债权包</span>
                	...
                	<li>
                	<span class="s_login_txt">&nbsp;</span>
                	  <input type="button" value="   提      交   "  onclick="chanage_Buton()" class="input_c"/>
                	</li>
                 </ul>
 ...
 //密码
  ...
  //注册按钮;
   function reg_Buton(){
       ...
       //param["paramMap.cellcode"] = $("#cellcode").val();//手机校验码
      
       $.post("loanAdminUserCellreginfo.do",param,function(data){
		    ...
		    success: function (data) {
		    	...
		    	if(data.result != null && data.result != undefined && data.result != ''){
		    		...
		    		for(var i=0;i<data.result.length;i++){
	                    ...
	                    strHtml +="<td align='center' >"+o.mobilePhone+"</td>";
	                    ...
	                    if(o.isFlag==0){
	                    	 strHtml +="<td class='f66' align='center' height='36px' w>"+'普通用户'+"</td>";
	                    }
	                    if(o.isFlag==1){
	                    	 strHtml +="<td class='f66' align='center' height='36px' >"+'高级用户'+"</td>";
	                    }
	                    //strHtml +="<td class='f66' align='center' height='36px'>"+'<a href="###" onclick="editeUser('+"'"+o.admin_user_id+"'"+','+"'"+o.isFlag+"'"+');">编辑</a>'+"</td>";
						strHtml +="</tr>";
			    	}
   ...
   function editeUser(admin_user_id,isFlag){
	   var param = {};
	   param['admin_user_id'] = admin_user_id;
	   param['isFlag'] = isFlag;
	   
	  if(confirm('你确定要重新设置用户超级状态吗？')){
		  $.ajax({
			    type: "post",
			    url: "updateAdminUser.do",
			    dataType: "json",
			    data:param,
			    success: function (data) {
			       if(data.status==1){
   ...
   function chanage_Buton(){
	   var param = {};
	   param['userName_check'] = $('#userName_check').val();
	   param['password_check'] = $('#password_check').val();
	   param['isFlag_check'] = $('#isFlag_check').val();
	  
	      $.ajax({
			    type: "post",
			    url: "updateAdminUserCheck.do",
			    dataType: "json",
			    data:param,
			    success: function (data) {
			       if(data.status==1){
