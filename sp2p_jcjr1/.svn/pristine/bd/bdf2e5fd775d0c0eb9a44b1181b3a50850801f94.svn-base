...
public class OnlinePaymentAction extends BaseFrontAction {
	...
	public String alipayPayment() throws Exception {
		...
		if (nunber != -1) {
			Map<String, String> map = rechargeService.getRechargeDetail(nunber);
	...
	private String createUrl(String out_trade_no, String body,
		...
		sPara.put("subject", IConstants.PRO_GLOBLE_NAME+"充值编号:" + out_trade_no);
	...
	public String alipayReceive() throws Exception {
		...
		String extra_common_param = request.getString("extra_common_param");// 获得参数信息
		// 支付类型_订单编号/金钱_用户编号
		...
		String sellerEmail = request.getString("seller_email");// 商户邮箱
		if (!sellerEmail.equals(AlipayConfig.seller_email)) {// 比较商户邮箱看是否符合
			createHelpMessage("支付失败！", "返回首页", "index.do");
		}
		String paynumber = URLDecoder.decode(request.getString("trade_no"), "utf-8");
		// 支付宝编号
		String notify_time = URLDecoder.decode(request.getString("notify_time"), "utf-8");// 支付宝编号
		String paybank = null;// 支付银行
		if (StringUtils.isBlank(paybank)) {// 如果没有银行编号说明是支付宝直接支付的
			paybank = "支付宝余额支付";
		}
		String buyer_email = URLDecoder.decode(request.getString("buyer_email"), "utf-8");// 支付银行
		String buyer_id = URLDecoder.decode(request.getString("buyer_id"), "utf-8");// 支付银行
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("extraCommonParam", extraCommonParam);
		map.put("total_fee", new BigDecimal(request.getString("total_fee")));
		...
		if (returnId < 0) {
			...
			sendMessage(userId, request.getDouble("total_fee", 0), 0);
			// ------------------
	...
	private void sendMessage(Long userId, double total, double money)
		...
		try {
			...
			if (lists != null && lists.size() > 0) {
				String content = "你已成功从"+IConstants.PRO_GLOBLE_NAME+"充值￥" + total + "元，扣除手续费后到账金额为￥"
						+ money + "元，请注意查收!";
