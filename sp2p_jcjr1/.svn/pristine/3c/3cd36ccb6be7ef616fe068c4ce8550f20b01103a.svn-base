...
public class RiskManageDao {
	...
	*/
	...
	public Map<String, String> queryLianlianProvinceCode(Connection conn,String provinceName) throws SQLException, DataException {
	    StringBuffer command = new StringBuffer();
	    command.append(" SELECT  * from t_lianian_province_city_code where province_name LIKE '"+provinceName+"%'" );
		DataSet dataSet =MySQL.executeQuery(conn, command.toString());
		command=null;
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	
	public Map<String, String> queryLianlianCityCode(Connection conn,String cityName) throws SQLException, DataException {
	    StringBuffer command = new StringBuffer();
	    command.append(" SELECT  * from t_lianian_province_city_code where city_name LIKE '"+cityName+"%'" );
		DataSet dataSet =MySQL.executeQuery(conn, command.toString());
		command=null;
		return BeanMapUtils.dataSetToMap(dataSet);
	}
