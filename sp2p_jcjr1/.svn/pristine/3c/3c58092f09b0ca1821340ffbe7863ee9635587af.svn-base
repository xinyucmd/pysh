...
public class AdminService extends BaseService {
	 ...
	 * @throws DataException
	 */
	public Long addLoanAdmin(String userName, String password, int enable,
			long roleId, String realName, String telphone, String qq,
			String email, String img, String isLeader,Long loanId) throws Exception {
		Connection conn = MySQL.getConnection();
		...
		try {
			...
			adminId = addAdminInfo(conn, userName, password, enable, roleId,realName, telphone, qq, email, img, isLeader);
			companyLoanDao.updateCompnayLoan(conn, loanId, adminId);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public Admin adminLogin(String userName, String password, String IP)
		...
		adminmap = adminDao.queryAdminByName(conn, userName);
		if (null == adminmap) {
			return null;
		}
		long id = Convert.strToLong(adminmap.get("id"), 0);
		String lastLoginTime = Convert.strToStr(adminmap.get("lastTime"), null);
		...
		if(null!=lastLoginTime){
			...
			if (new Date().getTime() - loginTime.getTime() > 3*60* 60 * 1000) {
				...
				adminDao.resetAdminState(conn, 0, 1, id);
				conn.commit();
			}
		}
		adminmap = adminDao.queryAdminByName(conn, userName);
		int loginErrorCount = Convert.strToInt(adminmap.get("loginErrorCount"),
				0);
		int isLoginLimit = Convert.strToInt(adminmap.get("isLoginLimit"), 1);
		if (loginErrorCount == 3 || isLoginLimit == 2) {
			
			// 设置用户限制登录，设置loginErrorCount为0
			adminDao.resetAdminState(conn, 0, 2, id);
			conn.commit();
			admin = new Admin();
...
//			
			...
			if (returnId <= 0) {
				...
				// 更新用户错误登录次数和登陆时间
				adminDao.updateAdminState(conn, count,simple.format(new Date()) , id);
				conn.commit();
				return null;
			} else {
				adminDao.resetAdminState(conn, 0, 1, id);
	...
	public long queryAllAdmin() throws Exception {
		Connection conn = MySQL.getConnection();
		Map<String, String> map = new HashMap<String, String>();
		long num = 0;
		try {
			map = adminDao.queryAllAdmin(conn);
			num = Convert.strToLong(map.get("num"), 0);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public Map<String, String> queryAdmin() throws Exception {
		Connection conn = MySQL.getConnection();
		Map<String, String> map = new HashMap<String, String>();
		try {
			map = adminDao.queryAdmin(conn);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	 ...
	 * @throws Exception
	 */
	public Map<String, String> queryUTool() throws Exception {
		Connection conn = MySQL.getConnection();
		Map<String, String> map = new HashMap<String, String>();
		try {
			map = adminDao.queryUtool(conn);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public Map<String, String> queryAdminByName(String userName) throws Exception {
		Connection conn = MySQL.getConnection();
		Map<String, String> map = new HashMap<String, String>();
		try {
			map = adminDao.queryAdminByName(conn,userName);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	 ...
	 * @throws Exception
	 */
	public Map<String, String> queryAdminByInfo(String userName,String password) throws Exception {
		Connection conn = MySQL.getConnection();
		Map<String, String> map = new HashMap<String, String>();
		try {
			map = adminDao.queryIdByInfo(conn,userName,password);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public long updateAdminPass(String password) throws Exception {
		Connection conn = MySQL.getConnection();
		long result = -2L;
		if ("1".equals(IConstants.ENABLED_PASS)) {
		...
		try {
			result = adminDao.updateAdminPass(conn, password);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
