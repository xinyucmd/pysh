...
public class FundManagementService extends BaseService {
	...
	public Map<String, String> queryRechargeRecordList(PageBean<Map<String, Object>> pageBean,
			...
			int rechargeType, Integer result) throws Exception {
		...
		try {
			...
			DataSet ds = MySQL.executeQuery(conn, "select sum(rechargeMoney) as rechargeMoneys," +
					" sum(realMoney) as realMoneys,rechargeNumber from v_t_user_rechargedetails_list where 1=1 " + command.toString());
			conn.commit();
	...
	public Map<String, String> queryAllWithdrawList(PageBean<Map<String, Object>> pageBean,
		...
		try {
			dataPage(conn, pageBean, "v_t_user_withdraw_lists", "*", "",
					command);
			//String siteName = (String)ServletConfig.this.getServletContext().getAttribute("siteName");
	...
	public Map<String, String> queryUserCashList(PageBean<Map<String, Object>> pageBean,
		...
		String command2 = "";
		if (userName != null) {
			command += " and username like '%"+ StringEscapeUtils.escapeSql(userName.trim()) + "%'"+" or mobilePhone like '%"+ StringEscapeUtils.escapeSql(userName.trim()) + "%'";
			command2 += " where a.username like '%"+ StringEscapeUtils.escapeSql(userName.trim()) + "%'"+" or a.mobilePhone like '%"+ StringEscapeUtils.escapeSql(userName.trim()) + "%'";
		}
		StringBuffer cmd = new StringBuffer();
		cmd
				.append("(select a.id as userId,a.username,a.mobilePhone,IFNULL(f.forRePaySum,0) as dueoutSum,a.usableSum,a.freezeSum,");
		cmd
				.append("sum(IFNULL(b.recivedPrincipal+b.recievedInterest-b.hasPrincipal-b.hasInterest,0)) as dueinSum,d.realName realName from t_user a left join t_invest b on a.id = b.investor ");
		...
		cmd
				.append(" on f.publisher = a.id "+command2+" group by a.ID,a.usableSum,a.freezeSum,f.forRePaySum,d.realName,a.username) u");
		try {
			...
			DataSet ds = MySQL.executeQuery(conn, "select sum(u.dueoutSum) as dueoutSums, sum(u.usableSum) as usableSums," +
					" sum(u.freezeSum) as freezeSums, sum(u.dueinSum) as dueinSums from "+cmd.toString());
			conn.commit();
			return BeanMapUtils.dataSetToMap(ds);
		} catch (Exception e) {
			log.error(e);
	...
	public Long addBackR(Long userId, Long adminId, Integer type, double money,
		...
		try {
			...
			if (ret > 0) {
				...
				informTemplate = informTemplate.replace("money", new BigDecimal(money).setScale(2, BigDecimal.ROUND_UP) + "");
				...
				e_informTemplate = e_informTemplate
						.replace("money", new BigDecimal(money).setScale(2, BigDecimal.ROUND_UP) + "");
	...
	public Map<String, String> updateWithdrawTransfer(Long wid, Integer status,
			Long adminId, String ipAddress, String checkremark) throws Exception {
		ipAddress = Utility.filteSqlInfusion(ipAddress);
		...
		try {
			Procedures.p_amount_withdraw_transfer(conn, ds, outParameterValues,
					adminId, wid, status, ipAddress, "", checkremark, -1, "");
			result = Convert.strToLong(outParameterValues.get(0) + "", -1);
			map.put("ret", outParameterValues.get(0) + "");
			map.put("ret_desc", outParameterValues.get(1) + "");
			if (result <= 0) {
	...
	public Map<String, String> queryOneBankCardInfoApply(Long bankId)
			throws Exception {
		Connection conn = MySQL.getConnection();

		try {
			return userBankDao.queryOneBankCardInfoApply(conn, bankId, -1, -1);
		} catch (Exception e) {
			conn.rollback();
			log.error(e);
			e.printStackTrace();
	...
	public Long updateBankBaseInfo(Long bankId, String bankName,
			String subBankName,String provinceCode,String cityCode) throws Exception {
		Connection conn = MySQL.getConnection();
		Long result = -1L;
		try {

			result = userBankDao.updateBankBaseInfo(conn, bankId, bankName,subBankName,provinceCode,cityCode);
			conn.commit();
		} catch (Exception e) {
			conn.rollback();
			log.error(e);
	...
	public Long updateChangeBank(Long bankId, String bankName,
		...
		try {
			...
			result = userBankDao.updateChangeBankInfo(conn, bankId, bankName,subBankName, bankCard, status, date, modify);
			conn.commit();
		} catch (Exception e) {
			conn.rollback();
			log.error(e);
	...
	//银行卡变更申请
	public Long updateChangeBank_apply(Long bankId, String bankName,
			String subBankName, String bankCard, int status, Date date,
			boolean modify,String bank_card_path,String identify_path) throws Exception {
		Connection conn = MySQL.getConnection();
		Long result = -1L;
		try {

			result = userBankDao.updateChangeBankInfo(conn, bankId, bankName,subBankName, bankCard, status, date, modify);
			Map<String,String> map = userBankDao.queryBankApplyInfo(conn, bankId);
			if(map!=null && map.size()>0){
				long id = Long.parseLong(map.get("id"));
				userBankDao.updateBankApplyInfo(conn, id, bankId, bank_card_path, identify_path);
			}else{
				userBankDao.addBankApplyInfo(conn, bankId, bank_card_path, identify_path);
			}
	...
	public void queryRiskByCondition(String resource, String timeStart,
			String timeEnd, String riskType, PageBean<Map<String, Object>> pageBean)
			throws Exception {
	...
	public Map<String, String> queryLianlianProvinceCode(String provinceName) throws Exception {
		Connection conn = MySQL.getConnection();
		Map<String, String> map = new HashMap<String, String>();
		try {
			map = riskManageDao.queryLianlianProvinceCode(conn, provinceName);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public Map<String, String> queryLianlianCityCode(String cityName) throws Exception {
		Connection conn = MySQL.getConnection();
		Map<String, String> map = new HashMap<String, String>();
		try {
			map = riskManageDao.queryLianlianCityCode(conn, cityName);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public Map<String, String> queryPlatformCost() throws Exception,
		DataException {
		Connection conn = MySQL.getConnection();
		// 手机变更状态不为空
		try {
			return fiManageDao.queryPlatformCost(conn);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
