						...
						<table style="margin-bottom: 8px;" cellspacing="0" cellpadding="0"
							...
							<tbody>
								 ...
								 <tr>
									<td class="f66" align="left" width="25%" height="36px">
										充值类型：&nbsp;&nbsp;
								   		<s:if test="%{paramMap.rechargeType==1}">支付宝支付</s:if>
										<s:if test="%{paramMap.rechargeType==2}">环迅支付</s:if>
										<s:if test="%{paramMap.rechargeType==3}">国付宝</s:if> 
										<s:if test="%{paramMap.rechargeType==4}">线下充值</s:if>
										<s:if test="%{paramMap.rechargeType==5}">手工充值</s:if>
										<s:if test="%{paramMap.rechargeType==6}">虚拟充值</s:if>
										<s:if test="%{paramMap.rechargeType==7}">奖励充值</s:if>
									</td>
								</tr>
								<!-- 
								<tr>
								...
								<tr>
									<td class="f66" align="left" width="25%" height="36px">
										流水号：&nbsp;&nbsp;
										  <s:property  value="paramMap.id"></s:property>
