		...
		<form id="BaseDataform">
			<table class="s_basedata">
                    	<tr>
							...
							<span style="color: red; float: none; font-size:12px;" id="u_realName"
								class="formtips"></span>
					    </s:if>
						<s:else>
						  ${map.realName}
						...
						<s:if test='#request.map.realName==""'>
							<td width="300"><a href="javascript:void();" onclick="shezhi('realName')">设置</a><span class="font-red">(设置后不可修改，请认真核实)</span></td>
						</s:if><s:else>
                        ...
                        <tr>
                        	<th>身份证号：</th>
                            <td><s:if test='#request.map.idNo==""'>
													<input type="text" class="test s_inputval" name="paramMap.idNo"
														id="idNo" />
													<span style="color: red; float: none; font-size:12px;" id="u_idNo"
														class="formtips"></span>
												</s:if>
												<s:else><a id='idNo1'></a>
                            ...
                            <td><s:if test='#request.map.idNo==""'><a href="javascript:void();" onclick="shezhi('idNo')">设置</a><span class="font-red">(设置后不可修改，请认真核实)</span>
                            </s:if><s:else><span class="font-gray">完成</span></s:else></td>
                        </tr>
                        <tr>
                        	<th>手机绑定：</th>
                            <td>
                        ...
                        <tr>
                        	<th>邮箱绑定：</th>
                            <td><span class="un-set"><s:if test="#request.email==null or #request.email==''">未设置</s:if>
                            <s:else>${email }</s:else></span></td>
