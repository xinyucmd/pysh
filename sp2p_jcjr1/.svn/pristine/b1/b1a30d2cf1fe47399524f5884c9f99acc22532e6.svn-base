...
public class FrontMyPaymentDao {
	...
	public Map<String,String> queryMyPayData(Connection conn,long payId,int limitStart,int limitCount) throws SQLException, DataException{
		...
		command.append(" a.stillInterest as stillInterest, ");
		command.append(" b.annualRate as annualRate");
		command.append(" FROM t_repayment a LEFT JOIN t_borrow b ON a.borrowId=b.id LEFT JOIN t_user c ON b.publisher=c.id where a.id="+payId+" limit 0,1");
