...
public class MessageAppAction extends BaseAppAction {
	...
	public String sendMsg() throws IOException {
		...
		try {
			...
			if(!receiver.equals("admin")&&map == null){
				jsonMap.put("error", "5");
				jsonMap.put("msg", "收件人不存在");
				JSONUtils.printObject(jsonMap);
			}else if(receiver.equals("admin")){
				receiverId = -1L;
			}else if(map != null){
				receiverId = Convert.strToLong(map.get("id"), -2l);
			}
	...
	public String queryMsgDetail() throws SQLException, DataException, IOException {
		...
		try {
			...
			if (IConstants.MAIL_UN_READ == Convert.strToInt(msgMap
					.get("mailStatus"), -1)) {// 如果是未读信息，则更新数据库，将状态改为已读
				homeInfoSettingService.updateMails(mailId + "",
						IConstants.MAIL_READED);

			}
			String content = msgMap.get("mailContent");
			...
			if(content.contains("<a")){
				...
				if(s.length>1){
					...
					msgMap.put("mailContent",s[0]+""+s2[0]+""+s1[2] );
				}
			}
			jsonMap.put("error", "-1");
			jsonMap.put("msg", "成功");
