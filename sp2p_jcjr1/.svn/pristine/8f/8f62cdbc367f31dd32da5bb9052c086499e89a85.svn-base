...
public class MyHomeInfoSettingDao {
	...
	public Map<String, String> getDealPwd(Connection conn,long userId,int limitStart,int limitCount) throws SQLException, DataException{
		Dao.Tables.t_user tuser = new Dao().new Tables().new t_user();
		DataSet dataSet = tuser.open(conn, "*", " id="+userId, "", limitStart, limitStart);
	...
	public Long addBankCardInfo(Connection conn,long userId,String cardUserName,String bankName,String subBankName,
			String bankCard,int cardStatus,String p_code,String c_code) throws SQLException{
		...
		t_bank.unique_mark.setValue(1);
		t_bank.province_code.setValue(p_code);
		t_bank.city_code.setValue(c_code);
...
//	}
	...
	public List<Map<String,Object>> queryBankInfoList(Connection conn,long userId,int limitStart,int limitCount)
		...
		DataSet dataSet = t_bankCard.open(conn, "  modifiedBankName,modifiedBranchBankName,modifiedCardNo, cardUserName ,bankName,branchBankName,cardNo,modifiedCardStatus,id,cardStatus,unique_mark   ", " userId='"+userId+"'", " ", limitStart, limitCount);
		dataSet.tables.get(0).rows.genRowsMap();
		return dataSet.tables.get(0).rows.rowsMap;
	}
...
//		String condition = " userId='"+userId+"' and cardStatus = 1";
	 ...
	 * @throws DataException
	 */
	public Map<String,String> queryBankCard(Connection conn,String bankCard)throws SQLException, DataException{
		StringBuffer sql = new StringBuffer();
		sql.append(" select * from t_bankcard a where a.cardNo = '"+bankCard+"'");
	...
	*/
	 ...
	 * @Descb: 添加用户反馈意见
	 */
	public Long addOptions(Connection conn, long userId, String title, String options) throws SQLException {
		Dao.Tables.t_options_feedback t_options_feedback = new Dao().new Tables().new t_options_feedback();
		t_options_feedback.userId.setValue(userId);
		t_options_feedback.title.setValue(title);
		t_options_feedback.options.setValue(options);
		return t_options_feedback.insert(conn);
	}
