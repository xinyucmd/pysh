...
public class BeVipDao {
	 ...
	 * 查询用户的手机
	 */
	public Map<String, String> queryIsPhone(Connection conn, String phone)
			throws SQLException, DataException {
		 phone=com.shove.web.Utility.filteSqlInfusion(phone);
		Dao.Tables.t_person person = new Dao().new Tables().new t_person();
		DataSet dataSet = person.open(conn, "cellPhone", " cellPhone = "
				+ StringEscapeUtils.escapeSql(phone), "", -1, -1);
	 ...
	 * 查询身份证号
	 */
	public Map<String, String> queryIsIdno(Connection conn, String IDNOM)
			throws SQLException, DataException {
		IDNOM=com.shove.web.Utility.filteSqlInfusion(IDNOM);
		Dao.Tables.t_person person = new Dao().new Tables().new t_person();
		DataSet dataSet = person.open(conn, "idNo", " idNo = '"
				+ StringEscapeUtils.escapeSql(IDNOM) + "'", "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	
	
	public Map<String, String> queryUserType(Connection conn,long userId)
			throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		command.append(" SELECT  * from t_group_user t where  t.groupId = 8 AND t.userId = "+userId);
	 ...
	 */
	public Map<String, String> queryIsPhoneonUser(Connection conn,
			String mobilePhone) throws SQLException, DataException {
		mobilePhone=com.shove.web.Utility.filteSqlInfusion(mobilePhone);
		Dao.Tables.t_user user = new Dao().new Tables().new t_user();
		DataSet dataSet = user.open(conn, "mobilePhone", " mobilePhone = '"
				+ StringEscapeUtils.escapeSql(mobilePhone.trim()) + "'", "",
				-1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	 ...
	 *            用户id
	 */
	public Map<String, String> queryPUser(Connection conn, Long id)
			throws SQLException, DataException {
		Dao.Tables.t_person person = new Dao().new Tables().new t_person();
		DataSet dataSet = person.open(conn, "id,cellphone,idNo", " userId = "
				+ id, "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	 ...
	 * @throws SQLException
	 */
	public Long updatePhoneBanding(Connection conn, Long id, int Status)
			throws SQLException {
	 ...
	 *            手机申请变更的id
	 */
	public Long delectPhone(Connection conn, Long tpiid, int Statuas,
	...
	// 更新用户的vip状态
	public Long updateUser(Connection conn, Long uerId, int vipStatus,
			int servicePersonId, String content, String vipFee, int authStep)
			throws SQLException, DataException {
		Dao.Tables.t_user user = new Dao().new Tables().new t_user();
	 ...
	 * @throws DataException
	 */
	public Long updateUserAustep(Connection conn, Long id, Integer austept)
			throws SQLException {
		Dao.Tables.t_user user = new Dao().new Tables().new t_user();
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryIDCard(Connection conn, String idCard)
			throws SQLException, DataException {
		idCard=com.shove.web.Utility.filteSqlInfusion(idCard);
		Dao.Tables.t_person person = new Dao().new Tables().new t_person();
		DataSet dataSet = person.open(conn, " idNo, userId", " idNo = '"
				+ StringEscapeUtils.escapeSql(idCard) + "'", "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	}
