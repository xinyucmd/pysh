...
public class DownloadDao {
	...
	public Map<String, String> frontGetDownloadPreById(Connection conn, Long id)
			throws SQLException, DataException {
		Dao.Views.v_t_download_detail download = new Dao().new Views().new v_t_download_detail();
		DataSet dataSet = download.open(conn, "", "id=(select max(id) from t_download where id < " + id+")", "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);

	}

	public Map<String, String> frontGetDownloadAfterById(Connection conn, Long id)
			throws SQLException, DataException {
		Dao.Views.v_t_download_detail download = new Dao().new Views().new v_t_download_detail();
		DataSet dataSet = download.open(conn, "", "id=(select min(id) from t_download where id > " + id+")", "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	...
	public List<Map<String, Object>> queryDownloadList(Connection conn,
			...
			throws SQLException, DataException {
		fiedList = Utility.filteSqlInfusion(fiedList);
		orderStr = Utility.filteSqlInfusion(orderStr);
		title = Utility.filteSqlInfusion(title);
		Dao.Tables.t_download download = new Dao().new Tables().new t_download();
		StringBuffer querydownload = new StringBuffer(" 1=1 ");
		title=com.shove.web.Utility.filteSqlInfusion(title);
		if (StringUtils.isNotBlank(title)) {
			querydownload.append(" AND title = "
