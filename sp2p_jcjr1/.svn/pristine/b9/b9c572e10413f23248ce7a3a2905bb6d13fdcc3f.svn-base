...
public class FinanceToolsService extends BaseService {
	 ...
	 *            以月为单位
	 */
	public List<Map<String, Object>> rateIncome2Month(double borrowSum,
			double yearRate, int borrowTime, double bidReward,
			double bidRewardMoney) {
		...
		double rewardSum = Double.parseDouble(df.format(borrowSum * bidReward
				/ 100))
				+ bidRewardMoney;
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("reward", String.format("%.2f", rewardSum));

		double netIncome = allSum
				- Double.valueOf(df.format(Double.valueOf(df.format(allSum
						- borrowSum)) * 0.1));
		 ...
		 * 年化收益
		 */
		double rateVal = ExcelRateUtil.excelRate((borrowSum - rewardSum),
				Double.parseDouble(df.format(monRepay)), monTime, 200, 15);
	 ...
	 * @return
	 */
	public Map<String, Object> rateCalculateDay(double borrowSum,
	 ...
	 * 按月还款。等额本息计算
	 */
	public List<Map<String, Object>> rateCalculate2Month(double borrowSum,
			double yearRate, int borrowTime) {
		if (borrowSum < 0 || yearRate < 0 || borrowTime < 0) {
			return null;
		}
		...
		try {
			for (int j = 1; j <= borrowTime; j++) {
				...
				monForA = Convert.strToDouble(df.format(monPay2 - monForRate),
						0);
				...
				payA += monForA;
				payRemain = Convert.strToDouble(df.format(payRemain - monPay2),
						0);
				if (j == borrowTime) {
					payRemain = 0;
				}
				// i*100 月利率以百分比显示
				map = addToMap(j, monPay2, monForA, monForRate, payRemain,
						Convert.strToDouble(df.format(i * 100), 0), Convert
								.strToDouble(df.format(allSum), 0));
				lists.add(map);
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @return
	 */
	public List<Map<String, Object>> rateCalculate2Sum(double borrowSum,
			double yearRate, int borrowTime) {
		if (borrowSum < 0 || yearRate < 0 || borrowTime < 0) {
			return null;
		}
		...
		try {
			for (int j = 1; j <= mon; j++) {
				...
				if (j == mon) {
					// 最后一个月还本金
					monForRateA = Convert.strToDouble(df.format(borrowSum
							+ monPayRate), 0);
					monForA = Convert.strToDouble(df.format(borrowSum), 0);
					payRemain = 0;
				}
				// i*100 月利率以百分比显示
				map = addToMap(j, monForRateA, monForA, monPayRate, payRemain,
						Convert.strToDouble((df.format(i * 100)), 0), Convert
								.strToDouble((df.format(allSum)), 0));
				lists.add(map);
			}
	 ...
	 * @return
	 */
	private Map<String, Object> addToMap(int mon, double monPay,
	 ...
	 * @return
	 */
	public List<Map<String, Object>> rateIncome2Sum(double borrowSum,
			double yearRate, int borrowTime, double bidReward,
			double bidRewardMoney) {
		if (borrowSum < 0 || yearRate < 0 || borrowTime < 0) {
			return null;
		 ...
		 * 年化收益
		 */
		double rateVal = ExcelRateUtil.rateTotal(allSum,
				(borrowSum - rewardSum), borrowTime);
	 ...
	 * @return
	 */
	public List<Map<String, Object>> rateIncome2Sum_new(double borrowSum,
			double yearRate, int borrowTime, double bidReward,
			double bidRewardMoney) {
		if (borrowSum < 0 || yearRate < 0 || borrowTime < 0) {
			return null;
		...
		map.put("reward", String.format("%.2f", rewardSum));
		map.put("allSum", String.format("%.2f", allSum));
		map.put("monPay", String.format("%.2f", 0.00));
		map.put("netIncome", String.format("%.2f", netIncome));
		map.put("rateSum", String.format("%.2f", allSum - borrowSum));
	 ...
	 * 一次还款
	 */
	public List<Map<String, Object>> rateCalculate2SumOne(double borrowSum,
			double yearRate, int borrowTime) {
		if (borrowSum < 0 || yearRate < 0 || borrowTime < 0) {
			return null;
		}
		...
		try {
			for (int j = 1; j <= mon; j++) {
				...
				if (j == mon) {
					...
					monForA = Convert.strToDouble(df.format(borrowSum), 0);
					payRemain = 0;
					monPayRate = Double.parseDouble(df.format(borrowSum * i
							* 0.01))
							* mon;
				}
				// i*100 月利率以百分比显示
				map = addToMap(j, monForRateA, monForA, monPayRate, payRemain,
						Convert.strToDouble((df.format(i)), 0), Convert
								.strToDouble((df.format(allSum)), 0));
				lists.add(map);
			}
		} catch (Exception e) {
	 ...
	 * 一次还款
	 */
	public List<Map<String, Object>> rateCalculate2SumOne_new(double borrowSum,
			double yearRate, int borrowTime) {
		if (borrowSum < 0 || yearRate < 0 || borrowTime < 0) {
			return null;
		}
			...
			for (int j = 1; j <= mon; j++) {
				...
				if (j == mon) {
					...
					monPayRate = Double.parseDouble(df.format(borrowSum * i
							...
							* mon;
					// i*100 月利率以百分比显示
					map = addToMap(j, monForRateA, monForA, monPayRate,
							payRemain, Convert.strToDouble((df.format(i)), 0),
	 ...
	 */
	public Map<String, String> getPhoneNumInfo(String phoneNum)
			throws Exception {
