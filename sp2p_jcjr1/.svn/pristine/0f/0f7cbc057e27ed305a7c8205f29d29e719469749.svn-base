...
public class ShoveBorrowTypeDao {
	 ...
	 * 增加标种类型
	 */
	public Long addShoveBoorowType(Connection  conn,String nid,int status,String name,String title,String description,double account_multiple,int password_status ,
					...
					int late_days_day,double vip_late_scale, double all_late_scale,double all_frost_scale) throws SQLException{
	...
	public Long  updateShoveBorrowType(Connection  conn,int id,int status,String title,String description,double account_multiple,int password_status ,
			...
			int late_days_day,double vip_late_scale, double all_late_scale,double all_frost_scale,int vorsion,String nid,
		...
		t_shove_borrow_type.amount_first.setValue(amount_first);
		t_shove_borrow_type.amount_end.setValue(amount_end);
		t_shove_borrow_type.identifier.setValue(nid);
		t_shove_borrow_type.apr_first.setValue(apr_first);
		t_shove_borrow_type.apr_end.setValue(apr_end);	
		...
		t_shove_borrow_type.all_late_scale.setValue(all_late_scale);
		t_shove_borrow_type.all_frost_scale.setValue(all_frost_scale);
		t_shove_borrow_type.version.setValue(vorsion);
	...
	public Map<String, String> queryShoveBorrowTypeByNid(Connection conn,
		...
		nid=com.shove.web.Utility.filteSqlInfusion(nid);
		Dao.Tables.t_borrow_type  t_shove_borrow_type = new Dao().new Tables().new t_borrow_type();
	 ...
	 * @throws DataException
	 */
	public List<Map<String, Object>> queryBorrowType(Connection  conn, String id) throws SQLException, DataException{
		Dao.Tables.t_borrow_type  t_borrow_type = new Dao().new Tables().new t_borrow_type();
		DataSet  ds= t_borrow_type.open(conn, " id, nid, name ", " status = "+ id, "", -1, -1);
