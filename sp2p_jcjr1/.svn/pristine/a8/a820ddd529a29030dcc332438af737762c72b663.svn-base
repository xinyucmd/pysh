					...
					<div style="width: auto; background-color: #FFF; padding: 10px;">
						...
						<s:iterator value="rightsList.{?#this.id<0}" var="bean">
							...
							<s:iterator value="rightsList.{?#this.parentID==#bean.id}" var="sBean">
								&nbsp;&nbsp;&nbsp;<input class="top_${bean.id} parent" name="rightsId" id="parent_${sBean.id }" parentId="${sBean.parentId }" type="checkbox" value="${sBean.id}" />
								${sBean.summary }<s:iterator value="rightsList.{?#this.parentID==#sBean.id}" var="sbBean">&nbsp;&nbsp;&nbsp;
								<input class="top_${bean.id} bottom parent_${sBean.id}" name="rightsId" parentId="${sbBean.parentId }"  type="checkbox" value="${sbBean.id}" />
