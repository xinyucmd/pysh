...
<div class="tabmain">
  ...
  <p align="right">
	 合同编号: <b><u>WXD—${map.DatesNumber}${map.id }</u></b>
</p>
<p align="right">
...
<p>
	贷出者:
	 <!-- 借款人可以看到所有投资人 -->
	 <s:if test="#request.map.userId==#request.userId">
		 <s:if test="#request.user_invest_map==null || #request.user_invest_map.size==0">
			     　暂无贷出者
		 </s:if>
		 <s:else>
			<s:iterator var="mapBeans" value="#request.user_invest_map" ><b><u>
				<s:if test="%{#mapBeans.realName==''}">
				    <shove:sub  value="#request.mapBeans.username" size="1"   suffix="**" />
				</s:if>
				<s:else>
				     <shove:sub  value="#request.mapBeans.realName" size="1"   suffix="**" /> 
				</s:else>
				</u></b><b><u>;</u></b> 
			</s:iterator>
		</s:else>
	 ...
	 <s:else>
		 <s:if test="#request.now_user_invest_map==null || #request.now_user_invest_map.size==0">
			     　暂无贷出者
		 </s:if>
		 <s:else>
		 	<b><u>
		 		<s:if test="#request.now_user_invest_map.realName==''">${now_user_invest_map.username}</s:if>
				<s:else><shove:sub  value="#request.now_user_invest_map.realName" size="1"   suffix="**" /></s:else>
			</u></b><b><u>;</u></b>
		 </s:else>
	 </s:else>
	 ，以下称“贷出方”；
...
<p>
	借入者: 
	<b><shove:sub  value="#request.map.realName" size="1"   suffix="**" />
	&nbsp;&nbsp;&nbsp; 
	(<shove:sub  value="#request.map.isno" size="12"   suffix="******" />)
	</b>，以下称“借入方”；
</p>
<p>
...
<p>
	...
	<!-- 借款人可以看到所有投资人 -->
	 ...
	 <s:else>
		 ...
		 <s:else>
		 	<b><u>
		 		<s:if test="#request.now_user_invest_map.realName==''">
		 		   <shove:sub  value="#request.now_user_invest_map.username" size="1"   suffix="**" />
		 		</s:if>
				<s:else>
				    <shove:sub  value="#request.now_user_invest_map.realName" size="1"   suffix="**" />
...
<p>
	借入者: <b>
	<shove:sub  value="#request.map.realName" size="1"   suffix="**" />
	        &nbsp;&nbsp;&nbsp; (<shove:sub  value="#request.map.isno" size="12"   suffix="******" />)</b>
</p>
<p>
	通信地址：<b><u>北京市海淀区</u></b>
</p>
