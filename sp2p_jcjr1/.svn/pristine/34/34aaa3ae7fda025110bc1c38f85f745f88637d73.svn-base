...
public class HelpAction extends BasePageAction {
	...
	*/
	public String addHelp()  throws Exception{
		...
		String title = Convert.strToStr(paramMap.get("title"),null); //标题
		...
		String content = Convert.strToStr(paramMap.get("content"), null); //详细内容
		String publisher = Convert.strToStr(paramMap.get("publisher"), null); //Convert.strToLong(paramMap.get("adminId"), -1); //发布人
...
//		
		...
		try {
			...
			if (result > 0) {
				...
				operationLogService.addOperationLog("t_help_question", admin.getUserName(),IConstants.INSERT, admin.getLastIP(), 0, "添加"+IConstants.PRO_GLOBLE_NAME+"IConstants.PRO_GLOBLE_NAME帮助问题数据", 2);
			}
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	* @return String
	*/
	public String updateHelpInit() throws Exception {
	...
	* @return String
	 * @throws DataException 
	*/
	public String updateHelp() throws Exception {
		String title = Convert.strToStr(paramMap.get("question"),null); //标题
		
		long id = Convert.strToLong(paramMap.get("questionId"),-1L); //序号不能改，问题Id
		long typeId = Long.valueOf(paramMap.get("typeId")); //帮助类型id
		String publisher = Convert.strToStr(paramMap.get("publisher"),null); //发布人
		String publishTime = Convert.strToStr(paramMap.get("publishTime"),null); //发布时间
		...
		try {
			...
			if (result > 0) {
				...
				Admin admin = (Admin) session().getAttribute(IConstants.SESSION_ADMIN);
				operationLogService.addOperationLog("t_help_question", admin.getUserName(),IConstants.UPDATE, admin.getLastIP(), 0, "修改"+IConstants.PRO_GLOBLE_NAME+"帮助问题数据成功", 2);
	...
	public String updateHelpManager () throws Exception {
		...
		String content = Convert.strToStr(paramMap.get("anwer"), null);
		String publisher = Convert.strToStr(paramMap.get("publisher"),null); //发布人
		String publishTime = Convert.strToStr(paramMap.get("publishTime"),null); //发布时间
	...
	* @return String
	*/
	public String deleteHelp() throws Exception {
		String helpIds = request.getString("commonId");
	...
	public String deleteHelpManager() throws Exception {
		String helpIds = request.getString("typeId");//这里只是单条信息的删除
