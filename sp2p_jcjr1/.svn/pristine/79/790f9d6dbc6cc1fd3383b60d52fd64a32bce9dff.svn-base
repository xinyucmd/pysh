...
//	}
	...
	public void queryUserManageInfo(PageBean<Map<String, Object>> pageBean,
			String userName, String realName,String createTimeMin,String createTimeMax) throws Exception {
		userName = Utility.filteSqlInfusion(userName);
		realName = Utility.filteSqlInfusion(realName);
		...
		try {
			...
			if (StringUtils.isNotBlank(createTimeMin)) {
				condition.append(" and createTime >= '"+createTimeMin+"'");
			}
			
			if (StringUtils.isNotBlank(createTimeMax)) {
				createTimeMax = createTimeMax+" 23:59:59";
				condition.append(" and createTime <= '"+createTimeMax+"'");
			}
