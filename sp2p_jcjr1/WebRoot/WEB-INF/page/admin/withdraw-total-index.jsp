...
<body>
	...
	<div style="padding: 15px 10px 0px 10px;">
    	...
    	<div class="right_title">
    	    <form id="withdrawForm" action="withdrawAll.do" method="post">
	          ...
	          <%--  <span>用户组：<s:select id="group" name="group" list="#{'1':'真实用户','2':'机构账户','3':'线下理财人'}"  headerKey="-1" headerValue="--请选择--"></s:select></span> --%>
	           <input name="" type="button"  value="查找" class="cen_btn" onclick="withdrawContions()"/>
	           <!-- id="theme-login" -->
        ...
        <div style="width:1200px; margin-top:20px;">
        	<table class="zh_table" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top:10px;">
                <tbody>
                    <tr>
                        <td>总累计提现金额<br/>（元）</td>
                        <td style="color:#ff0000">${allWithdrawUser }</td>
                        <td>总新增提现金额<br/>（元）</td>
                        <td style="color:#ff0000">${allNewWithdraw }</td>
                    </tr>
                     <tr>
                        <td>真实用户累计提现金额<br/>（元）</td>
                        <td style="color:#ff0000">${allWithdrawUser1 }</td>
                        
                        <td>真实用户新增提现金额<br/>（元）</td>
                        <td style="color:#ff0000">${allNewWithdraw1 }</td>
                    </tr>
                    <tr>
                        <td>机构用户累计提现金额<br/>（元）</td>
                        <td>${allWithdrawUser2 }</td>
                        <td>机构用户新增提现金额<br/>(元)</td>
                        <td>${allNewWithdraw2 }</td>
                    </tr> 
                    <tr>
                        <td>线下理财人累计提现金额<br/>（元）</td>
                        <td>${allWithdrawUser3 }</td>
                        <td>线下理财人新增提现金额<br/>（元）</td>
                        <td>${allNewWithdraw3 }</td>
                    </tr>
                </tbody>
           </table>	
        </div>
