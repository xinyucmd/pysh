			...
			<tbody>
				...
				<s:iterator value="pageBean.page" var="bean" status="s">
					...
					<td>
							...
							<s:elseif test="#bean.isWebRepay == 2">
							  是
							</s:elseif>
					</td>
					<td>
					...
					<td>
					     <s:if test="#bean.repayStatus == 1">
							<s:if test="#bean.isWebRepay == 1">
					          <a href="javascript:repaySubmit(${bean.id});">还款</a>
							</s:if>
						 </s:if>
				...
				<td colspan="2">￥<s:if test="#request.overduePaymentMap.totalSumm==''">0</s:if><s:else>${overduePaymentMap.totalSumm}</s:else></td>
				...
				<td colspan="2">￥<s:if test="#request.overduePaymentMap.totallateFI==''">0</s:if><s:else>${overduePaymentMap.totallateFI}</s:else></td>
				...
				<td colspan="2">￥<s:if test="#request.overduePaymentMap.amount==''">0</s:if><s:else>${overduePaymentMap.amount}</s:else></td>
				<td  colspan="2" align="right">当前页逾期还款金额：</td>
				<td></td><td></td>
				<td  align="center" ><fmt:formatNumber value="${overAmount }" type="number" pattern="￥0.00" /></td><td></td><td></td><td></td>
				</tr>
			</tbody>
		</table>
		<p/>
		<!--<span class="require-field">当前页逾期垫付金额:￥${overAmount }元</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<span class="require-field">逾期垫付总金额:￥<s:if test="%{#request.overduePaymentMap.amount==''}">0</s:if> <s:else><s:property value="#request.overduePaymentMap.amount" default="0"/></s:else>元</span><p/>
--></div>
