 ...
 * @date 2012-6-11 下午04:37:13
 */
public class NewEveryDayRate extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext arg0)
		...
		// 日终处理服务类
		DayRateServices dayRateServices = SpringFactory
				.getBean("dayRateServices");
		// 系统日期服务类
		SystemDateServices systemDateServices = SpringFactory
				.getBean("systemDateServices");
		// 催收费规则
		CollectFeeService collectFeeService = SpringFactory
				.getBean("collectFeeService");

		// 新结算相关服务类
		// 还款计划
		LoanRepayPlanBO loanRepayPlanBO = SpringFactory
				.getBean("loanRepayPlanBO");
		// 还款历史
		LoanRepayHisBO loanRepayHisBO = SpringFactory.getBean("loanRepayHisBO");
		// 流水
		AccountRepayRelBO accountRepayRelBO = SpringFactory
				.getBean("accountRepayRelBO");
		// 主账户
		LoanMainAccountBO loanMainAccountBO = SpringFactory
				.getBean("loanMainAccountBO");
		// 分账户
		LoanSubAccountBO loanSubAccountBO = SpringFactory
				.getBean("loanSubAccountBO");
		...
		String systemDate = DateUtil.addByDay(SystemParm.SystemDate.trim(), 1,
				DateUtil.DATE_FORMAT_);
...
//		parmAcLnMstBean.setDueNo("LX1033713002015063017167001");
		parmAcLnMstBean.setDueState("1");
		// 查询出有那些借据需要进行批量处理
		List<AcLnMstBean> acLnMstBeans = repayServiceImpl
				.getAcLnMstBeans(parmAcLnMstBean);
		...
		for (AcLnMstBean acLnMstBean : acLnMstBeans) {
			...
			for (LoanSubAccount subBean : subAccountList) {
				if ("21".equals(subBean.getAccount_type())) {
					dueBal = BigNumberUtil.Subtract(subBean.getShould_pay(),
							subBean.getPay());
				}
			}
			...
			for (LoanRepayPlan plan : planBeans) {
				if ("3".equals(plan.getIf_settle())) {// 已还清
					if (Integer.parseInt(lastFinishTerm) <= Integer
							.parseInt(plan.getTerm())) {
			...
			// 当剩余本金为0时不再进行批量计算
			if (Double.parseDouble(dueBal) == 0
					|| Double.parseDouble(dueBal) < 0) {
				...
				if (sqfx) {
					...
					if ("B".equals(settleModel)) {
						// 最后一期逾期实行22.4
						overrate = Double.valueOf(acLnMstBean.getLastRate());
						String overInterest = getPenalty_15(overrate,dayRateServices,  systemDate,acLnMstBean, planBeans, loanRepayPlanBO,repayServiceImpl, dueBal,loanMainAccountBO,mainAccount.getAccount_no());
						...
						ratedayBean.setCsf(csf);
					} else if ("C".equals(settleModel)
							|| "D".equals(settleModel)) {
						overrate = Double.valueOf(acLnMstBean.getLastRate());
						String overInterest = getPenalty_15LX(overrate,dayRateServices, planServices, systemDate,acLnMstBean, planBeans, loanRepayPlanBO,repayServiceImpl, dueBal,loanMainAccountBO,mainAccount.getAccount_no());
						ratedayBean.setOverInterest(overInterest);
						String csf = getPenalty_2(overrate, dayRateServices,planServices, collectFeeService, systemDate,acLnMstBean, loanRepayPlanBO, planBeans, dueBal);
						ratedayBean.setCsf(csf);
					}
			...
			if (planBean == null && planBeans.size() == 1) {
				ratedayBean.setCapital(planBeans.get(0).getCapital());
				// 设置日终利息
				ratedayBean.setInterest(planBeans.get(0).getInterest());
				// 设置管理费
				ratedayBean.setAccFee(planBeans.get(0).getOther());
				// 设置日终期号
				ratedayBean.setTermNo("1");
			} else {
				if (planBean == null) {
					if (DateUtil.gt(currDate, planBeans.get(0).getBeg_date())) {
						planBean = planBeans.get(0);
					} else {
						planBean = planBeans.get(planBeans.size() - 1);
					}

				}
				ratedayBean.setCapital(planBean.getCapital());
				// 设置日终利息
				ratedayBean.setInterest(planBean.getInterest());
				// 设置管理费
				ratedayBean.setAccFee(planBean.getOther());
				// 设置日终期号
				ratedayBean.setTermNo(planBean.getTerm());
			}
			// 设置发生日期
			...
			String lastDate = DateUtil.addByDay(currDate, -1,
					DateUtil.DATE_FORMAT_);
			parmRatedayBean.setOccureDate(lastDate);
			RatedayBean lastRatedayBean = dayRateServices
					.getRateDay(parmRatedayBean);
			// 获取上一个日终结束
			RatedayBean ratedayBean_0 = new RatedayBean();
			ratedayBean_0.setDueNo(dueNo);
			List<RatedayBean> ratedayBeans = dayRateServices
					.getRateDayBeanList(ratedayBean_0);

			if (planBeans.size() > 1 && ratedayBeans.size() > 0
					&& lastRatedayBean == null) {
				continue;
			}
			// 上一个日终的期号
			...
			if (planBeans.size() > 1
					&& !StringUtil.equals(lastTermNo, planBean.getTerm())) {
	...
	public String getPenalty_15LX(double lnRate,
			DayRateServices dayRateServices, PlanServices planServices,
			String systemDate, AcLnMstBean acLnMstBean,
			List<LoanRepayPlan> planBeans, LoanRepayPlanBO loanRepayPlanBO,
			IRepayService repayServiceImpl, String dueBal, LoanMainAccountBO loanMainAccountBO, String accountNo) {
		String dueno = acLnMstBean.getDueNo();
		...
		for (LoanRepayPlan plan : planBeans) {
			if ("3".equals(plan.getIf_settle())) {// 已还清
				if (Integer.parseInt(lastFinishTerm) <= Integer.parseInt(plan
						.getTerm())) {
					lastFinishTerm = plan.getTerm();
				}
			}
		...
		// 3.判断2、systemDate的先后顺序，得出结论是否需要计算罚息
		// nextTermDate>systemDate==>nextTermDate+1
		// lastReturnDate = DateUtil.addByDay(nextTermDate.replace("-", ""), 1);

		StringBuffer fx = new StringBuffer();
		// lastReturnDate = lastReturnDate.substring(0,
		// 4)+"-"+lastReturnDate.substring(4, 6)+"-"+lastReturnDate.substring(6,
		// 8);
		int day = DateUtil.getBetweenDays(nextTermDate, systemDate);
		...
		LoanMainAccount account = new LoanMainAccount();
		account.setAccount_no(accountNo);
		account.setOver_days(String.valueOf(day));
		loanMainAccountBO.update(account);
		if (day < 0) {
			return "0";
		}

		String baseMoney = dueBal;
		// 计算费用收取基数
		...
		fx.append("借据号(" + dueno + ")#发生日期" + systemDate + "#" + systemDate
				+ "-" + lastReturnDate + "=" + day + "天#");
		fx.append(baseMoney);
		fx.append("×");
		fx.append(day);
		fx.append("×");
		...
		String overins = BigNumberUtil.Multiply(acLnMstBean.getDueBal(),
				overrate);
		...
		RatedayBean ratedayBean = new RatedayBean();
		ratedayBean.setDueNo(acLnMstBean.getDueNo());
		String lastDate = DateUtil.addByDay(systemDate, -1,
				DateUtil.DATE_FORMAT_);
		...
		try {
			...
			System.out.println(acLnMstBean.getDueNo()+"chucuole!!");
			System.out.println(systemDate+"chucuole!!");
			e.printStackTrace();
		}
	...
	public String getPenalty_15(double lnRate,
			DayRateServices dayRateServices, 
			String systemDate, AcLnMstBean acLnMstBean,
			List<LoanRepayPlan> planBeans, LoanRepayPlanBO loanRepayPlanBO,
			IRepayService repayServiceImpl, String dueBal, LoanMainAccountBO loanMainAccountBO, String accountNo) {
		String dueno = acLnMstBean.getDueNo();
		...
		String overrate = String.valueOf(lnRate);
		// 比较日期是否在期内,获取逾期利率
//		if (DateUtil.gt(acLnMstBean.getDueEndDate(), systemDate)) {
//			overrate = acLnMstBean.getLastRate();
//		} else {
...
//		}
		...
		pb.setDue_no(dueno);
		List<LoanRepayPlan> pblist = loanRepayPlanBO.viewPlan(pb);
		if(pblist!=null && pblist.size()>0){
			Collections.sort(pblist);
			
			for (LoanRepayPlan x : pblist) {
		...
		if (DateUtil.gteq(systemDate, pblist.get(pblist.size()-1).getEnd_date())) {// 如果最后一期逾期之后将不再减去当日普通利息
	...
	// overrate, dayRateServices, planServices, collectFeeService,systemDate,
	// acLnMstBean, planBeans, loanRepayPlanBO,planBeans
	public String getPenalty_2(double lnRate, DayRateServices dayRateServices,
			PlanServices planServices, CollectFeeService collectFeeService,
			String systemDate, AcLnMstBean acLnMstBean,
			LoanRepayPlanBO loanRepayPlanBO, List<LoanRepayPlan> planBeans,
			String dueBal) {
		String csf = "0.00";
		String lastReturnDate = "";
		String dueno = acLnMstBean.getDueNo();
		...
		if (day > 60) {
			...
			CollectFeePolicy p = collectFeeService.getById(acLnMstBean
					.getCollectfeeId());
			String fee = "0.00";
			// 到期前每月2%
			if (!DateUtil.gt(acLnMstBean.getDueEndDate(), systemDate)) {
				...
				// 到期后首月3%, 之后每月递增1%
				int overlastdays = DateUtil.getBetweenDays(
						acLnMstBean.getDueEndDate(), systemDate);
	 ...
	 */
	public int[] getOverduePerioddays(String dueno, String termno,
			DayRateServices dayRateServices, String systemDate) {
		PlanBean planbean = new PlanBean();
		planbean.setDueNo(dueno);
		List<PlanBean> planbeanList = dayRateServices
				.getOverdueplanBeans(planbean);// 获取一共逾期一共几期，还过款和没有还过款的都包括
		int countdays = 0;// 逾期天数
		int j = 0;// 逾期期数
		for (PlanBean planBean : planbeanList) {
			if (Integer.valueOf(termno) >= Integer
					.valueOf(planBean.getTermNo())) {
				if (planBean.getIsDebt() != null
						&& Integer.valueOf(planBean.getIsDebt()) > 0) {
					countdays = countdays
							+ Integer.valueOf(planBean.getIsDebt());
					j++;
				} else if (planBean.getState().equals("1")
						|| (planBean.getState().equals("0") && DateUtil
								.getBetweenDays(planBean.getEndDate(),
										systemDate) > 0)) {
					countdays = countdays
							+ DateUtil.getBetweenDays(planBean.getEndDate(),
									systemDate);
					j++;
				}
			}
	 ...
	 * 获取计算罚息的时开始日期
	 */
	private String getOverLastTerm(List<RepayBean> repayBeans,
		...
		// 当期的期供信息
		String sum0 = BigNumberUtil.Add(planBean.getReturnCapital(),
				planBean.getReturnInterest());
		// 当期偿还的期供和期供的差值
		...
		for (RepayBean repayBean : repayBeans) {
			sumReturn = BigNumberUtil.Add(sumReturn,
					repayBean.getReturnCapital(),
					repayBean.getReturnInterest(), repayBean.getBjbalance(),
					repayBean.getRatecomebalance());
		}

		Double d2 = Double.parseDouble(sumReturn);

		for (PlanBean planBean_ : PlanBeans) {
			String termReturn = BigNumberUtil.Add(planBean_.getReturnCapital(),
					planBean_.getReturnInterest());
			Double d1 = Double.parseDouble(termReturn);
			d2 = d2 - d1;
			d2 = Double.parseDouble(BigNumberUtil.Divide2(String.valueOf(d2),
					"1"));
			if (d2 < 0) {
	 ...
	 * 获取某一期的还款信息
	 */
	private List<String> getReturnOfTerm(List<RepayBean> repayBeans,
			String termNo) {
		...
		for (RepayBean repayBean : repayBeans) {
			if (StringUtil.equals(repayBean.getTermNo().trim(), termNo.trim())) {
				repaytotal = BigNumberUtil.Add(repaytotal,
						repayBean.getReturnCapital(),
						repayBean.getReturnInterest());
				repayBalance = BigNumberUtil.Add(repayBalance,
						repayBean.getBjbalance(),
						repayBean.getRatecomebalance());
			}
		}
