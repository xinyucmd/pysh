...
(function($) {
    $.addFlex = function(t, p) {
        ...
        p = $.extend({
            ...
            procmsg: '正在处理数据，请稍候 ...', //正在处理的提示信息<b><font color="red">Processing, please wait ...</font></b>
            query: '', //搜索查询的条件
            qtype: '', //搜索查询的类别
            qop: "Eq", //搜索的操作符
            ...
            onSubmit: false, // using a custom populate function,调用自定义的计算函数，基本没用
                            ...
                            $(ths).each(function(j) {
                                ...
                                if (idx == "-1") { //checkbox
                                	...
                                	if (typeof(row.cell[idx])!= "undefined") { 
                                		parm = parm+row.cell[idx]+"#"
                                	}
                                	
                                	if(idx==lasttd){
	
                                		divInner="<a href='#' onclick=myclick('"+parm+"')>连接</a>";
                                	
                                	}else{
                                		 var divInner = row.cell[idx] || "&nbsp;";
                                    	 if (this.process) {
                                         	divInner = this.process(divInner, trid,row);
                                    	 }
                                	}
                                    //设置td显示的文本内容
                                    div.push(divInner);
        ...
        // 创建没有记录时存放提示信息的div
        	var left = (p.width-176)/2;
        	left = parseInt(left);
        	var $mesdiv = $('<div id="emptymes"  style="position: absolute;top:50%;left:'+left+'px"></div>');
         	$mesdiv.appendTo('body');
  		// 创建没有记录时存放提示信息的div
