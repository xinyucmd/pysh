    ...
    $(document).ready(function(){
      var entityName = '<%=(String)request.getAttribute("entityName")%>';	
  	  $("#remove").click(function() {
   		  many_removerow('/system/unselectUser.action?relationKey=' + entityName + '&relationValue=<s:property value="id" />&seleteIDs=');
   	  });

  	  $("#select").click(function() {
  		  openwindow2('/system/selectUserPage.action?relationKey=' + entityName + '&relationValue=<s:property value="id" />',750,500);
   	  });
     ...
     $('#tt').datagrid({
		...
		height:350,
		idField:'id', 
		url:'relate' + entityName + 'User.action?id=<s:property value="id" />',
		columns:[[
...
<body>
  <div id="feature">
    <div id="shortcuts" class="headerList">
      <span style="white-space: nowrap;"> <a id="remove" href="#"
