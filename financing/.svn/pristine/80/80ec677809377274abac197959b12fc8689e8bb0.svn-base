	  ...
	  var mygrid = jQuery("#grid").jqGrid({
		...
		{  
		   var par='<%=((User)session.getAttribute("loginUser")).getUpdate_account()%>';
		     new_format_value = "<a href='printPact.action?id=" + rowObject[0] + "'><s:text name="查看合同"/></a>";
		...
		{  
		   var par='<%=((User)session.getAttribute("loginUser")).getUpdate_account()%>';
		     new_format_value = "<a href='downloadPact.action?id=" + rowObject[0] + "'><s:text name="下载合同"/></a>";
		   return new_format_value
		};	
