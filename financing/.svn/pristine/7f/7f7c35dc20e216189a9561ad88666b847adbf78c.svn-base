...
<script type="text/javascript">
    $(document).ready(function(){
	  $("#delete").click(function() {	
		  many_deleterow("/investor/deleteInvestor.action?seleteIDs=");
	  });	

	  $("#massUpdate").click(function() {	
		  many_massUpdaterow("/investor/editAccount.action?seleteIDs=");
	  });
	  
	  $("#export").click(function() {	
		  many_exportrow("/investor/exportAccount.action?seleteIDs=");
	  });		  
	  
	  $("#copy").click(function() {	
		  many_copyrow("/investor/copyAccount.action?seleteIDs=");
	  });

	  var mygrid = jQuery("#grid").jqGrid({
			datatype: "json", 
			url:'listInvestorFull.action', 
			mtype: 'POST',
			height: "auto",
		...
		{  
		   ...
		   if (par == 1){
		     new_format_value = "<a href='editInvestor.action?id=" + rowObject[0] + "'>" + cellvalue + "</a>";
		   }else {
   ...
   <div id="shortcuts" class="headerList">
    <b style="white-space: nowrap; color: #444;"><s:text
      ...
      <span style="white-space: nowrap;"> <a
       href="editInvestor.action" class="easyui-linkbutton"
      ...
      <div id="mtm1" style="width: 150px;">
       <s:if
        ...
        <div data-options="iconCls:'icon-import'"
         onClick="openwindow('/financing/upload.jsp?entityName=Account&namespace=investor&title=' + '<s:text name="title.import.account" />')">
