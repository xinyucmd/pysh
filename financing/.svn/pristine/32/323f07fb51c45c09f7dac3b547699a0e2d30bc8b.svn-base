...
<script type="text/javascript">
    $(document).ready(function(){
	  $("#delete").click(function() {	
		  many_deleterow("/crm/deleteMeeting.action?seleteIDs=");
	  });	

	  $("#massUpdate").click(function() {	
		  many_massUpdaterow("/crm/editMeeting.action?seleteIDs=");
	  });
	  
	  $("#export").click(function() {	
		  many_exportrow("/crm/exportMeeting.action?seleteIDs=");
	  });

	  $("#copy").click(function() {	
		  many_copyrow("/crm/copyMeeting.action?seleteIDs=");
	  });
	  ...
	  var mygrid = jQuery("#grid").jqGrid({
			...
			height: "auto",
		   	colNames:['<s:text name="entity.id.label" />','<s:text name="entity.subject.label" />',
		  		   	'<s:text name="entity.status.label" />','<s:text name="entity.start_date.label" />',
		  		   	'<s:text name="entity.end_date.label" />','<s:text name="meeting.location.label" />',
		   	...
		   	colModel:[
		   		...
		   		{name:'end_date',index:'end_date', width:150, resizable:true, formatter:urlFmatter, stype:'select', 
			   		editoptions:{value:"<%=DateTimeUtil.getSelectOptions()%>"}},
			   	{name:'location',index:'location', width:150, resizable:true, formatter:urlFmatter},
		   	...
		   	rowList:[15,50,100], 
		   	multiselect: true, 
		   	caption: "<s:text name='title.grid.meetings'/>"
		});
    ...
    <div id="feature">
      ...
      <div id="shortcuts" class="headerList">
        <b style="white-space: nowrap; color: #444;"><s:text
            ...
            test="#session.loginUser.create_meeting == 1">
            <span style="white-space: nowrap;"> <a
              href="editMeeting.action" class="easyui-linkbutton"
              iconCls="icon-add" plain="true"><s:text
                  name="action.createMeeting" /></a>
            </span>
          </s:if> <s:if test="#session.loginUser.delete_meeting == 1">
            <span style="white-space: nowrap;"> <a id="delete"
              href="#" class="easyui-linkbutton" iconCls="icon-remove"
              plain="true"><s:text name="action.deleteMeeting" /></a>
            </span>
            ...
            <div id="mtm1" style="width: 150px;">
              <s:if
                test="#session.loginUser.create_meeting == 1 || #session.loginUser.update_meeting == 1">
                <div data-options="iconCls:'icon-import'"
                  onClick="openwindow('/crm/upload.jsp?entityName=Meeting&namespace=crm&title=' + '<s:text name="title.import.meeting" />')">
              ...
              <s:if test="#session.loginUser.update_meeting == 1">
                <div data-options="iconCls:'icon-update'"
                  id="massUpdate">
                  <s:text name='menu.item.massupdate.title' />
                </div>
              </s:if>
              <s:if test="#session.loginUser.create_meeting == 1">
