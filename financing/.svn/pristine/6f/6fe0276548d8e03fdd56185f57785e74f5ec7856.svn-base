    ...
    $(document).ready(function(){
     ...
     if (message!=null&&message!='') {
	  ...
	  var mygrid = jQuery("#grid").jqGrid({
			datatype: "json", 
			url:'listContractGen.action', 
			mtype: 'POST',
			height: "auto",
		   	colNames:['<s:text name="entity.id.label" />','<s:text name="合同编号"/>','<s:text name="customer.cifName.label" />','<s:text name="customer.idNo.label" />',
		   	...
		   	colModel:[
                ...
                {name:'pactNo',index:'pactNo', width:120, resizable:true, formatter:urlFmatter},
		...
		{  
		   var par='<%=((User)session.getAttribute("loginUser")).getUpdate_account()%>';
		     new_format_value = "<a href='editContractGen.action?id=" + rowObject[0] + "'>" + cellvalue + "</a>";
