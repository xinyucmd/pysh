...
<script type="text/javascript">
    $(document).ready(function(){
	  $("#delete").click(function() {	
		  many_deleterow("/crm/deleteOpportunity.action?seleteIDs=");
	  });	

	  $("#massUpdate").click(function() {	
		  many_massUpdaterow("/crm/editOpportunity.action?seleteIDs=");
	  });
	  
	  $("#export").click(function() {	
		  many_exportrow("/crm/exportOpportunity.action?seleteIDs=");
	  });	
	  
	  $("#copy").click(function() {	
		  many_copyrow("/crm/copyOpportunity.action?seleteIDs=");
	  });
	  
	  var mygrid = jQuery("#grid").jqGrid({
			datatype: "json", 
			url:'listOpportunityFull.action', 
			mtype: 'POST',
			height: "auto",
		   	colNames:['<s:text name="entity.id.label" />','<s:text name="entity.name.label" />',
		  		   	'<s:text name="entity.account.label" />','<s:text name="menu.salesStage.title" />',
		  		   	'<s:text name="opportunity.opportunity_amount.label" />','<s:text name="entity.assigned_to.label" />',
		   	...
		   	colModel:[
		   		...
		   		{name:'sales_stage.name',index:'sales_stage.name', width:150, resizable:true, formatter:urlFmatter},
		   		{name:'opportunity_amount',index:'opportunity_amount', width:150, resizable:true, formatter:urlFmatter},
		   	...
		   	rowList:[15,50,100], 
		   	multiselect: true, 
		   	caption: "<s:text name='title.grid.opportunities'/>"
		});
		function urlFmatter (cellvalue, options, rowObject)
		{  
		   var par='<%=((User)session.getAttribute("loginUser")).getUpdate_opportunity()%>';
		   if (par == 1){
			   new_format_value = "<a href='editOpportunity.action?id=" + rowObject[0] + "'>" + cellvalue + "</a>";
		   }else {
    ...
    <div id="feature">
      ...
      <div id="shortcuts" class="headerList">
        <b style="white-space: nowrap; color: #444;"><s:text
            ...
            test="#session.loginUser.create_opportunity == 1">
            <span style="white-space: nowrap;"> <a
              href="editOpportunity.action" class="easyui-linkbutton"
              iconCls="icon-add" plain="true"><s:text
                  name="action.createOpportunity" /></a>
            </span>
          </s:if> <s:if test="#session.loginUser.delete_opportunity == 1">
            <span style="white-space: nowrap;"> <a id="delete"
              href="#" class="easyui-linkbutton" iconCls="icon-remove"
              plain="true"><s:text name="action.deleteOpportunity" /></a>
            </span>
            ...
            <div id="mtm1" style="width: 150px;">
              <s:if
                test="#session.loginUser.create_opportunity == 1 || #session.loginUser.update_opportunity == 1">
                <div data-options="iconCls:'icon-import'"
                  onClick="openwindow('/crm/upload.jsp?entityName=Opportunity&namespace=crm&title=' + '<s:text name="title.import.opportunity" />')">
              ...
              <s:if test="#session.loginUser.update_opportunity == 1">
                <div data-options="iconCls:'icon-update'"
                  id="massUpdate">
                  <s:text name='menu.item.massupdate.title' />
                </div>
              </s:if>
              <s:if test="#session.loginUser.create_opportunity == 1">
      ...
      <div id="feature-title">
        <h2>
          <s:text name="title.listOpportunity" />
        </h2>
      </div>
