...
public abstract class BaseListAction extends ActionSupport {
	...
	protected SearchCondition getSearchCondition(String name, User loginUser, String ownerId, String ortherCondition) throws Exception {
		StringBuffer condition = new StringBuffer("");
		...
		if (Constant.BOOLEAN_TRUE.equals(_search)) {
			...
			//客户经理
			if ("R02".equals(loginUser.getRole())) {
				if (!CommonUtil.isNullOrEmpty(ownerId)) {
					if (condition.length() != 0) {
						condition.append(" AND ");
					}
					condition.append(ownerId).append("=").append(loginUser.getId());
				}
			}
			
			//自定义查询条件
			if (!CommonUtil.isNullOrEmpty(ortherCondition)) {
				if (condition.length() != 0) {
					condition.append(" AND ");
				}
				condition.append(ortherCondition);
			}
		}
	...
	protected SearchCondition getSearchCondition(
		...
		if (scope == Role.OWNER_OR_DISABLED) {
			...
			condition.append("user_id = ").append(loginUser.getId());
		}
	...
	//add by wangyuqi 添加按登录用户查询 与 自定义查询条件
	@SuppressWarnings("rawtypes")
	protected SearchCondition getSearchCondition(
			Map<String, String> fieldTypeMap, User loginUser, String ownerId,String ortherCondition)
			throws Exception {
		HttpServletRequest request = ServletActionContext.getRequest();
