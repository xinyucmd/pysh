...
<script type="text/javascript">
    $(document).ready(function(){
	  $("#delete").click(function() {	
		  many_deleterow("/crm/deleteCall.action?seleteIDs=");
	  });	
    ...
    <div id="feature">
      ...
      <div id="shortcuts" class="headerList">
        <b style="white-space: nowrap; color: #444;"><s:text
            ...
            test="#session.loginUser.create_call == 1">
            <span style="white-space: nowrap;"> <a
              href="editCall.action" class="easyui-linkbutton"
              iconCls="icon-add" plain="true"><s:text
                  name="action.createCall" /></a>
            </span>
          </s:if> <s:if test="#session.loginUser.delete_call == 1">
            <span style="white-space: nowrap;"> <a id="delete"
              href="#" class="easyui-linkbutton" iconCls="icon-remove"
              plain="true"><s:text name="action.deleteCall" /></a>
            </span>
            ...
            <div id="mtm1" style="width: 150px;">
              <s:if
                test="#session.loginUser.create_call == 1 || #session.loginUser.update_call == 1">
                <div data-options="iconCls:'icon-import'"
                  onClick="openwindow('/crm/upload.jsp?entityName=Call&namespace=crm&title=' + '<s:text name="title.import.call" />')">
              ...
              <s:if test="#session.loginUser.update_call == 1">
                <div data-options="iconCls:'icon-update'"
                  id="massUpdate">
                  <s:text name='menu.item.massupdate.title' />
                </div>
              </s:if>
              <s:if test="#session.loginUser.create_call == 1">
