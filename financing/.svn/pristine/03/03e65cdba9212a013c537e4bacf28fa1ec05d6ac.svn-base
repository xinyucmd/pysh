...
<script type="text/javascript">
    $(document).ready(function(){
	  $("#delete").click(function() {	
		  many_deleterow("/crm/deleteContact.action?seleteIDs=");
	  });	

	  $("#massUpdate").click(function() {	
		  many_massUpdaterow("/crm/editContact.action?seleteIDs=");
	  });
	  ...
	  var mygrid = jQuery("#grid").jqGrid({
			...
			height: "auto",
		   	...
		   	rowList:[15,50,100], 
		   	multiselect: true, 
		   	caption: "<s:text name='title.grid.contacts'/>"
		});
		function urlFmatter (cellvalue, options, rowObject)
		{  
		   var par='<%=((User)session.getAttribute("loginUser")).getUpdate_contact()%>';
		   if (par == 1){
			   new_format_value = "<a href='editContact.action?id=" + rowObject[0] + "'>" + cellvalue + "</a>";
		   }else {
    ...
    <div id="feature">
      ...
      <div id="shortcuts" class="headerList">
        <b style="white-space: nowrap; color: #444;"><s:text
            ...
            test="#session.loginUser.create_contact == 1">
            <span style="white-space: nowrap;"> <a
              href="editContact.action" class="easyui-linkbutton"
              iconCls="icon-add" plain="true"><s:text
                  name="action.createContact" /></a>
            </span>
          </s:if> <s:if test="#session.loginUser.delete_contact == 1">
            <span style="white-space: nowrap;"> <a id="delete"
              href="#" class="easyui-linkbutton" iconCls="icon-remove"
              plain="true"><s:text name="action.deleteContact" /></a>
            </span>
            ...
            <div id="mtm1" style="width: 150px;">
              <s:if
                test="#session.loginUser.create_contact == 1 || #session.loginUser.update_contact == 1">
                <div data-options="iconCls:'icon-import'"
                  onClick="openwindow('/crm/upload.jsp?entityName=Contact&namespace=crm&title=' + '<s:text name="title.import.contact" />')">
              ...
              <s:if test="#session.loginUser.update_contact == 1">
                <div data-options="iconCls:'icon-update'"
                  id="massUpdate">
                  <s:text name='menu.item.massupdate.title' />
                </div>
              </s:if>
              <s:if test="#session.loginUser.create_contact == 1">
