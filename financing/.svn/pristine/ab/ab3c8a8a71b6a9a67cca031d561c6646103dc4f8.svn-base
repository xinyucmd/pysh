...
<script type="text/javascript">
    $(document).ready(function(){
    	$("#delete").click(function() {	
  		  many_deleterow("/contract/deletePackage.action?seleteIDs=");
  	  });
 
	  
	  var mygrid = jQuery("#grid").jqGrid({
			datatype: "json", 
			url:'listTemplatePackageFull.action', 			
			mtype: 'POST',
			height: "auto",
		   	colNames:['<s:text name="id" />',
		   	          '<s:text name="menu.ProductConfiguration.PADT_NO" />',
		  		 	  '<s:text name="模板包名称" />',
		  		 	 	'<s:text name="是否默认" />',
		  		 	  '<s:text name="是否有效" />'
		  	    	   ],
		   	colModel:[
				{name:'id',index:'id', width:100, resizable:true, key:true,hidden:true,formatter:urlFmatter},
		   		...
		   		{name:'defaulFlg',index:'defaulFlg', width:100, resizable:true, formatter:urlFmatter,stype:'select', 
		...
		{  
		   ...
		   if (par == 1){
		     new_format_value = "<a href='listTemplate.action?id=" + rowObject[0] + "'>" + cellvalue + "</a>";
		   }else {
     ...
     <div id="shortcuts" class="headerList">
        <b style="white-space: nowrap; color: #444;"><s:text
          ...
          style="white-space: nowrap;"> <a id="add"
            href="addPackage.action" class="easyui-linkbutton"
