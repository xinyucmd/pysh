...
<script type="text/javascript">
    $(document).ready(function(){
	  ...
	  var mygrid = jQuery("#grid").jqGrid({
			datatype: "json", 
			url:'listsubscripFull.action', 
			mtype: 'POST',
			height: "auto",
		   	colNames:['<s:text name="entity.id.label" />','<s:text name="customer.cifName.label" />','<s:text name="customer.idNo.label" />',
		...
		{  
		   var par='<%=((User)session.getAttribute("loginUser")).getUpdate_account()%>';
		     ...
		     new_format_value = "<a href='editSubscrip.action?id=" + rowObject[0] + "'>" + cellvalue + "</a>";
   ...
   <div id="shortcuts" class="headerList">
    <b style="white-space: nowrap; color: #444;"><s:text
      ...
      <span style="white-space: nowrap;"> <a
       href="editSubscrip.action" class="easyui-linkbutton"
