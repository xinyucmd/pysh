...
<script type="text/javascript">
    $(document).ready(function(){
	  $("#delete").click(function() {	
		  many_deleterow("/crm/deleteTarget.action?seleteIDs=");
	  });	

	  $("#massUpdate").click(function() {	
		  many_massUpdaterow("/crm/editTarget.action?seleteIDs=");
	  });
	  
	  $("#export").click(function() {	
		  many_exportrow("/crm/exportTarget.action?seleteIDs=");
	  });

	  $("#copy").click(function() {	
		  many_copyrow("/crm/copyTarget.action?seleteIDs=");
	  });
	  		  	  
	  var mygrid = jQuery("#grid").jqGrid({
			datatype: "json", 
			url:'listTargetFull.action', 
			mtype: 'POST',
			height: "auto",
		   	colNames:['<s:text name="entity.id.label" />','<s:text name="entity.first_name.label" />',
		   	        ...
		   	        '<s:text name="entity.account.label" />','<s:text name="entity.office_phone.label" />',
		   	        '<s:text name="entity.email.label" />','<s:text name="entity.assigned_to.label" />',
		   	...
		   	rowList:[15,50,100], 
		   	multiselect: true, 
		   	caption: "<s:text name='title.grid.targets'/>"
		});
    ...
    <div id="feature">
      ...
      <div id="shortcuts" class="headerList">
        <b style="white-space: nowrap; color: #444;"><s:text
            ...
            test="#session.loginUser.create_target == 1">
            <span style="white-space: nowrap;"> <a
              href="editTarget.action" class="easyui-linkbutton"
              iconCls="icon-add" plain="true"><s:text
                  name="action.createTarget" /></a>
            </span>
          </s:if> <s:if test="#session.loginUser.delete_target == 1">
            <span style="white-space: nowrap;"> <a id="delete"
              href="#" class="easyui-linkbutton" iconCls="icon-remove"
              plain="true"><s:text name="action.deleteTarget" /></a>
            </span>
            ...
            <div id="mtm1" style="width: 150px;">
              <s:if
                test="#session.loginUser.create_target == 1 || #session.loginUser.update_target == 1">
                <div data-options="iconCls:'icon-import'"
                  onClick="openwindow('/crm/upload.jsp?entityName=Target&namespace=crm&title=' + '<s:text name="title.import.target" />')">
              ...
              <s:if test="#session.loginUser.update_target == 1">
                <div data-options="iconCls:'icon-update'"
                  id="massUpdate">
                  <s:text name='menu.item.massupdate.title' />
                </div>
              </s:if>
              <s:if test="#session.loginUser.create_target == 1">
