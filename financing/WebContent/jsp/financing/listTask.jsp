...
<script type="text/javascript">
    $(document).ready(function(){
	  ...
	  var mygrid = jQuery("#grid").jqGrid({
			datatype: "json", 
			url:'listTaskFull.action', 
			mtype: 'POST',
			height: "auto",
		   	colNames:['<s:text name="entity.id.label" />','<s:text name="entity.subject.label" />',
		  		   	'<s:text name="entity.contact.label" />','<s:text name="entity.related_object.label" />',
		  		   	'<s:text name="task.due_date.label" />','<s:text name="entity.assigned_to.label" />',
		   	...
		   	colModel:[
		   		...
		   		{name:'contact.name',index:'contact.name', width:150, resizable:true, formatter:urlFmatter},
		   		{name:'related_object',index:'related_object', width:150, resizable:true, formatter:urlFmatter},
		   		{name:'due_date',index:'due_date', width:150, resizable:true, formatter:urlFmatter, stype:'select', 
		   	...
		   	rowList:[15,50,100], 
		   	multiselect: true, 
		   	caption: "<s:text name='title.grid.tasks'/>"
		});
    ...
    <div id="feature">
      ...
      <div id="shortcuts" class="headerList">
        <b style="white-space: nowrap; color: #444;"><s:text
            ...
            test="#session.loginUser.create_task == 1">
            <span style="white-space: nowrap;"> <a
              href="editTask.action" class="easyui-linkbutton"
              iconCls="icon-add" plain="true"><s:text
                  name="action.createTask" /></a>
            </span>
          </s:if> <s:if test="#session.loginUser.delete_task == 1">
            <span style="white-space: nowrap;"> <a id="delete"
              href="#" class="easyui-linkbutton" iconCls="icon-remove"
              plain="true"><s:text name="action.deleteTask" /></a>
            </span>
            ...
            <div id="mtm1" style="width: 150px;">
              <s:if
                test="#session.loginUser.create_task == 1 || #session.loginUser.update_task == 1">
                <div data-options="iconCls:'icon-import'"
                  onClick="openwindow('/crm/upload.jsp?entityName=Task&namespace=crm&title=' + '<s:text name="title.import.task" />')">
