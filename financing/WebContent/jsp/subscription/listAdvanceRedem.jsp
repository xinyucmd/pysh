...
<script type="text/javascript">
    $(document).ready(function(){
	  ...
	  var mygrid = jQuery("#grid").jqGrid({
			datatype: "json", 
			url:'listAdvanceRedem.action', 
			mtype: 'POST',
			height: "auto",
		   	colNames:['<s:text name="entity.id.label" />',
		   	          '<s:text name="customer.cifName.label" />','<s:text name="customer.idNo.label" />',
				  		'<s:text name="contact.tel" />',
				  		'<s:text name="subscript.prdt" />',
				  		'<s:text name="subscript.amt" />',
				  		'<s:text name="subscript.term" />',
		...
		{   
		   var par='<%=((User)session.getAttribute("loginUser")).getUpdate_account()%>';
		     new_format_value = "<a href='editAdvRedemConfirm.action?id=" + rowObject[0] + "'>" + cellvalue + "</a>";
